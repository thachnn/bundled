...
function argumentTypeError(observedType, allowedTypes, position, optional) {
  throw new YError(
    `Invalid ${positionName[position] || 'manyeth'} argument. Expected ${allowedTypes.join(' or ')} but received ${observedType}.`
  )
}
...
function parse(args, opts) {
  ...
  function setKey(obj, keys, value) {
    ...
    keys.slice(0, -1).forEach(function (key, _index) {
      key = sanitizeKey(key)
...
convert.rgb.lab = function (rgb) {
	...
	return [
		...
		500 * (x - y),
		200 * (y - (z > 0.008856 ? Math.pow(z, 1 / 3) : 7.787 * z + 16 / 116))
	];
};
...
convert.hsv.hsl = function (hsv) {
	...
	return [h, (sl / (lmin <= 1 ? lmin : 2 - lmin) || 0) * 100, 100 * (l / 2)];
};
...
convert.hex.rgb = function (/** number */ args) {
...
function Yargs(processArgs, cwd, parentRequire) {
   ...
   * @prop {*} narg
   * @prop {*} desc
   * @prop {*} demandedOptions
   * @prop {*} demandedCommands
   * @prop {*} hiddenOptions
   */
  let options
  self.resetOptions = self.reset = function (aliases) {
    ...
    objectOptions.forEach((k) => {
      tmpOptions[k] = objFilter(options[k], (k, _v) => !localLookup[k])
    })
  ...
  self.option = self.options = function (key, opt) {
    ...
    if (typeof key == 'object')
      ...
      opt.config && self.config(key, opt.config) // FIXED .configParser
...
module.exports = function (yargs, y18n) {
  ...
  self.reset = function (localLookup) {
    ...
    examples = []
    commands = []
    descriptions = objFilter(descriptions, (k, _v) => !localLookup[k])
    return self
  }
...
UI.prototype.toString = function () {
  ...
  _this.rows.forEach(function (row, _i) {
    _this.rowToString(row, lines)
  })

  return (lines = lines.filter(function (line) {
    return !line.hidden
  })).map(function (line) {
    return line.text
  }).join('\n')
...
module.exports = function (yargs, usage, y18n) {
  ...
  self.reset = function (localLookup) {
    implied = objFilter(implied, (k, _v) => !localLookup[k])
    conflicting = objFilter(conflicting, (k, _v) => !localLookup[k])
...
module.exports = function (_require) {
  var main = (_require || require).main
