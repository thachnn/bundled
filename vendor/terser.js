...
function defaults(args, defs, croak) {
  ...
  for (const i in defs) if (HOP(defs, i))
    if (!args || !HOP(args, i)) ret[i] = defs[i];
    else if (i === "ecma") {
      let ecma = args[i] | 0;
...
var MAP = (function() {
  function MAP(a, f, backwards) {
    ...
    function doit() {
      ...
      val instanceof AtTop
        ? (val = val.v) instanceof Splice ? top.push.apply(top, backwards ? val.v.slice().reverse() : val.v) : top.push(val)
        : val === skip || (val instanceof Splice ? ret.push.apply(ret, backwards ? val.v.slice().reverse() : val.v) : ret.push(val));
...
function make_node(ctor, orig, props) {
  props || (props = {});
  if (orig) {
    props.start || (props.start = orig.start);
    props.end || (props.end = orig.end);
  }
...
function mergeSort(array, cmp) {
  ...
  function merge(a, b) {
    ...
    ai < a.length && r.push.apply(r, a.slice(ai));
    bi < b.length && r.push.apply(r, b.slice(bi));
    return r;
  }
  function _ms(a) {
    if (a.length <= 1) return a;
    var m = Math.floor(a.length / 2), left = a.slice(0, m), right = a.slice(m);
    return merge(_ms(left), _ms(right));
  }
  return _ms(array);
}

function makePredicate(words) {
  Array.isArray(words) || (words = words.split(" "));
...
function regexp_source_fix(source) {
  return source.replace(/[\n\r\u2028\u2029]/g, function(match, offset) {
    var escaped = source[offset - 1] == "\\" &&
      (source[offset - 2] != "\\" || /(?:^|[^\\])(?:\\{2})*$/.test(source.slice(0, offset - 1)));
    return (escaped ? "" : "\\") + lineTerminatorEscape[match];
  });
}

const re_safe_regexp = /^[\\/|\0\s\w^$.[\]()]*$/,
  regexp_is_safe = source => re_safe_regexp.test(source),
...
function sort_regexp_flags(flags) {
  ...
  existing_flags.size && existing_flags.forEach(flag => { out += flag; });
...
var KEYWORDS_ATOM = "false null true",
  RESERVED_WORDS = "enum implements import interface package private protected public static super this " + KEYWORDS_ATOM + " " + KEYWORDS,
  KEYWORDS_BEFORE_EXPRESSION = "return new delete throw else case yield await";
...
var OPERATOR_CHARS = makePredicate(characters("+-*&%=<>!?|~^")),
  ...
  RE_NUM_LITERAL = /[0-9a-f]/i,
  RE_HEX_NUMBER = /^0x[0-9a-f]+$/i,
  RE_OCT_NUMBER = /^0[0-7]+$/,
  RE_ES6_OCT_NUMBER = /^0o[0-7]+$/i,
  RE_BIN_NUMBER = /^0b[01]+$/i,
  RE_DEC_NUMBER = /^\d*\.?\d*(?:e[+-]?\d*(?:\d\.?|\.?\d)\d*)?$/i,
  RE_BIG_INT = /^(0[xob])?[0-9a-f]+n$/i;
...
var NEWLINE_CHARS = makePredicate(characters("\n\r\u2028\u2029")),
  ...
  PUNC_AFTER_EXPRESSION = makePredicate(characters(";]),:")),
...
function get_full_char(str, pos) {
  ...
  } else if (is_surrogate_pair_tail(str.charCodeAt(pos)) && is_surrogate_pair_head(str.charCodeAt(pos - 1)))
...
function get_full_char_code(str, pos) {
  return is_surrogate_pair_head(str.charCodeAt(pos))
    ? 0x10000 + ((str.charCodeAt(pos) - 0xd800) << 10) + str.charCodeAt(pos + 1) - 0xdc00
    : str.charCodeAt(pos);
}
...
function from_char_code(code) {
  if (code > 0xffff) {
    code -= 0x10000;
    return String.fromCharCode(0xd800 + (code >> 10)) + String.fromCharCode((code % 0x400) + 0xdc00);
  }
...
function is_identifier_string(str, allow_surrogates) {
  if (/^[a-z_$][a-z0-9_$]*$/i.test(str)) return true;

  if (!allow_surrogates && /[\ud800-\udfff]/.test(str)) return false;
  ...
  return !(
    ...
    (!(match = UNICODE.ID_Continue.exec(str)) || match[0].length !== str.length))
  );
}
...
function tokenizer($TEXT, filename, html5_comments, shebang) {
  ...
  function find_eol() {
    for (var text = S.text, i = S.pos, n = text.length; i < n; ++i) {
      var ch = text[i];
  ...
  function token(type, value, is_comment) {
    S.regex_allowed =
      ...
      type == "arrow";
    type == "punc" && value == "." ? (prev_was_dot = true) : is_comment || (prev_was_dot = false);
  ...
  function read_while(pred) {
    var ret = "";
    for (var ch, i = 0; (ch = peek()) && pred(ch, i++); ) ret += next();
    return ret;
  }
  ...
  function read_num(prefix) {
    ...
    var num = read_while(function(ch, i) {
      ...
      case 111: case 79:
      case 120: case 88:
        return !has_x && (has_x = true);
      case 101: case 69:
        return !!has_x || (!has_e && (has_e = after_e = true));
      case 45:
    ...
    RE_OCT_NUMBER.test(num) && next_token.has_directive("use strict") &&
      parse_error("Legacy octal literals are not allowed in strict mode");

    if (numeric_separator) {
      num.endsWith("_") ? parse_error("Numeric separators are not allowed at the end of numeric literals")
        : num.includes("__") && parse_error("Only one underscore is allowed as numeric separator");

      num = num.replace(/_/g, "");
    }
    if (num.endsWith("n")) {
      const without_n = num.slice(0, -1),
        valid = parse_js_number(without_n, RE_HEX_NUMBER.test(without_n));
  ...
  function read_escaped_char(in_string, strict_hex, template_string) {
    ...
    case 117:
      if (peek() == "{") {
        next(true);
        peek() !== "}" || parse_error("Expecting hex-character between {}");
        while (peek() == "0") next(true);
        var result, length = find("}", true) - S.pos;
        if (length > 6 || (result = hex_bytes(length, strict_hex)) > 0x10ffff) parse_error("Unicode reference out of bounds");
    ...
    if (is_octal(ch)) {
      template_string && strict_hex && (ch !== "0" || is_octal(peek())) &&
  ...
  function read_octal_escape_sequence(ch, strict_octal) {
    var p = peek();
    if (p >= "0" && p <= "7" && (ch += next(true))[0] <= "3" && (p = peek()) >= "0" && p <= "7") ch += next(true);
  ...
  var read_template_characters = with_eof_error("Unterminated template", function(begin) {
    ...
    while ((ch = next(true, true)) != "`") {
      if (ch == "\r") {
        ...
        next(true, true);
        S.brace_counter++;
        (tok = token(begin ? "template_head" : "template_substitution", content)).raw = raw;
        return tok;
      }
      ...
      if (ch == "\\") {
        var tmp = S.pos;
        var prev_is_tag = previous_token && (previous_token.type === "name" ||
            (previous_token.type === "punc" && (previous_token.value === ")" || previous_token.value === "]")));
  ...
  var skip_multiline_comment = with_eof_error("Unterminated multiline comment", function() {
    var regex_allowed = S.regex_allowed,
      i = find("*/", true),
      text = S.text.substring(S.pos, i).replace(/\r\n|\r|\u2028|\u2029/g, "\n");
  ...
  var read_name = with_eof_error("Unterminated identifier name", function() {
    ...
    var read_escaped_identifier_char = function() {
      escaped = true;
      next();
      peek() === "u" || parse_error("Expecting UnicodeEscapeSequence -- uXXXX or u{XXXX}");

      return read_escaped_char(false, true);
    };

    if ((name = peek()) === "\\")
      is_identifier_start((name = read_escaped_identifier_char())) ||
        parse_error("First identifier char is an invalid identifier char");
    else if (is_identifier_start(name)) next();
    else return "";

    while ((ch = peek()) != null) {
      if ((ch = peek()) === "\\")
        is_identifier_char((ch = read_escaped_identifier_char())) || parse_error("Invalid escaped identifier char");
      else {
    ...
    RESERVED_WORDS.has(name) && escaped && parse_error("Escaped characters are not allowed in keywords");

    return name;
  });

  var read_regexp = with_eof_error("Unterminated regular expression", function(source) {
    for (var ch, prev_backslash = false, in_class = false; (ch = next(true)); )
      if (NEWLINE_CHARS.has(ch)) parse_error("Unexpected line terminator");
      ...
      else ch == "\\" ? (prev_backslash = true) : (source += ch);

    return token("regexp", { source, flags: read_name() });
  });
  ...
  function read_word() {
    var word = read_name();
    return prev_was_dot ? token("name", word)
  ...
  next_token.add_directive = function(directive) {
    ...
    S.directives[directive] === void 0 ? (S.directives[directive] = 1) : S.directives[directive]++;
  };
  ...
  next_token.pop_directives_stack = function() {
    for (var directives = S.directive_stack[S.directive_stack.length - 1], i = 0; i < directives.length; i++)
...
var PRECEDENCE = (function(a, ret) {
  for (var i = 0; i < a.length; ++i)
    for (var b = a[i], j = 0; j < b.length; ++j) ret[b[j]] = i + 1;
...
function parse($TEXT, options) {
  ...
  var statement = embed_tokens(function(is_export_default, is_for_body, is_if_body) {
    ...
    case "name":
      if (S.token.value == "async" && is_token(peek(), "keyword", "function")) {
        next();
        next();
        is_for_body && croak("functions are not allowed as the body of a loop");
    ...
    case "keyword":
      ...
      case "while":
        ...
        return new AST_While({
          ...
          body: in_loop(() => statement(false, true))
        });
      ...
      case "class":
        next();
        is_for_body && croak("classes are not allowed as the body of a loop");
        is_if_body && croak("classes are not allowed as the body of an if");
      ...
      case "return":
        S.in_function != 0 || options.bare_returns || croak("'return' outside of function");
        next();
        var value = null;
        if (is("punc", ";")) next();
      ...
      case "throw":
        next();
        has_newline_before(S.token) && croak("Illegal newline after 'throw'");
        value = expression(true);
        semicolon();
      ...
      case "with":
        S.input.has_directive("use strict") && croak("Strict mode may not include a with statement");
  ...
  function labeled_statement() {
    ...
    label.name === "await" && is_in_async() && token_error(S.prev, "await cannot be used as label inside async function");
    ...
    var stat = statement();
    S.labels.pop();
    stat instanceof AST_IterationStatement ||
  ...
  function break_cont(type) {
    var ldef, label = null;
    can_insert_semicolon() || (label = as_symbol(AST_LabelRef, true));

    if (label != null) {
      (ldef = S.labels.find(l => l.name === label.name)) || croak("Undefined label " + label.name);
      label.thedef = ldef;
    } else S.in_loop != 0 || croak(type.TYPE + " not inside a loop or switch");
    semicolon();
    var stat = new type({ label: label });
    ldef && ldef.references.push(stat);
    return stat;
  }
  ...
  function for_() {
    ...
    if (await_tok.type == "name" && await_tok.value == "await") {
      is_in_async() || token_error(await_tok, for_await_error);
    ...
    if (!is("punc", ";")) {
      ...
      var is_in = is("operator", "in"),
        is_of = is("name", "of");
      !await_tok || is_of || token_error(await_tok, for_await_error);

      if (is_in || is_of) {
        init instanceof AST_Definitions
          ? init.definitions.length > 1 && token_error(init.start, "Only one variable declaration allowed in for..in loop")
          : is_assignable(init) || (init = to_destructuring(init)) instanceof AST_Destructuring ||
            token_error(init.start, "Invalid left-hand side in for..in loop");

        next();
        return is_in ? for_in(init) : for_of(init, !!await_tok);
      }
    } else await_tok && token_error(await_tok, for_await_error);
  ...
  var arrow_function = function(start, argnames, is_async) {
    has_newline_before(S.token) && croak("Unexpected newline before arrow (=>)");

    expect_token("arrow", "=>");

    var body = _function_body(is("punc", "{"), false, is_async),

      end = body instanceof Array ? (body.length ? body[body.length - 1].end : start) : body.end;

    return new AST_Arrow({ start: start, end: end, async: is_async, argnames: argnames, body: body });
  };

  var function_ = function(ctor, is_generator_property, is_async, is_export_default) {
    var in_statement = ctor === AST_Defun,
      is_generator = is("operator", "*");
    is_generator && next();
    ...
    var args = [],
      body = _function_body(true, is_generator || is_generator_property, is_async, name, args);
    return new ctor({
      start: args.start,
      end: body.end,
    ...
    var tracker = {
      add_parameter: function(token) {
        if (parameters.has(token.value)) {
          if (duplicate === false) duplicate = token;
      ...
      mark_default_assignment: function(token) {
        if (default_assignment === false) default_assignment = token;
      },
      mark_spread: function(token) {
        if (spread === false) spread = token;
      },
      ...
      check_strict: function() {
        tracker.is_strict() && duplicate !== false && token_error(duplicate, "Parameter " + duplicate.value + " was used already");
      }
    };
  ...
  function parameters(params) {
    ...
    while (!is("punc", ")")) {
      ...
      if (!is("punc", ")")) {
        expect(",");
        is("punc", ")") && options.ecma < 2017 && unexpected();
      }
  ...
  function parameter(used_parameters, symbol_type) {
    var param,
      expand = false;
    if (used_parameters === void 0) used_parameters = track_used_binding_identifiers(true, S.input.has_directive("use strict"));
    ...
    if (is("operator", "=") && expand === false) {
      ...
      next();
      param = new AST_DefaultAssign({ start: param.start, left: param, operator: "=", right: expression(false), end: S.token });
    }

    if (expand !== false) {
      is("punc", ")") || unexpected();

      param = new AST_Expansion({ start: expand, expression: param, end: expand });
    }
  ...
  function binding_element(used_parameters, symbol_type) {
    var expand_token,
      ...
      is_expand = false,
      first_token = S.token;
    if (used_parameters === void 0) used_parameters = track_used_binding_identifiers(false, S.input.has_directive("use strict"));
    ...
    if (is("punc", "[")) {
      ...
      while (!is("punc", "]")) {
        ...
        if (is_expand) {
          is("punc", "]") || croak("Rest element must be last element");
      ...
      expect("]");
      used_parameters.check_strict();
      return new AST_Destructuring({ start: first_token, names: elements, is_array: true, end: prev() });
    }
    if (is("punc", "{")) {
      next();
      while (!is("punc", "}")) {
        ...
        if (is("name") && (is_token(peek(), "punc") || is_token(peek(), "operator")) && [",", "}", "="].includes(peek().value)) {
          used_parameters.add_parameter(S.token);
          var start = prev(),
            value = as_symbol(symbol_type);
          is_expand
            ? elements.push(new AST_Expansion({ start: expand_token, expression: value, end: value.end }))
            : elements.push(new AST_ObjectKeyVal({ start: start, key: value.name, value: value, end: value.end }));
        } else {
          if (is("punc", "}")) continue;
          var property_token = S.token,
            property = as_property_name();
          if (property === null) unexpected(prev());
          else if (prev().type === "name" && !is("punc", ":"))
            elements.push(new AST_ObjectKeyVal({
              start: prev(),
              key: property,
              value: new symbol_type({ start: prev(), name: property, end: prev() }),
              end: prev()
            }));
          else {
            expect(":");
        ...
        if (is_expand) is("punc", "}") || croak("Rest element must be last element");
        else if (is("operator", "=")) {
          used_parameters.mark_default_assignment(S.token);
          next();
      ...
      expect("}");
      used_parameters.check_strict();
      return new AST_Destructuring({ start: first_token, names: elements, is_array: false, end: prev() });
    }
    if (is("name")) {
      used_parameters.add_parameter(S.token);
  ...
  function params_or_seq_(allow_arrows, maybe_sequence) {
    ...
    expect("(");
    while (!is("punc", ")")) {
      spread_token && unexpected(spread_token);
      if (is("expand", "...")) {
    ...
    expect(")");
    allow_arrows && is("arrow", "=>")
      ? spread_token && trailing_comma && unexpected(trailing_comma)
      : invalid_sequence && unexpected(invalid_sequence);
  ...
  function _function_body(block, generator, is_async, name, args) {
    var loop = S.in_loop,
      labels = S.labels,
      current_generator = S.in_generator,
      current_async = S.in_async;
    ++S.in_function;
    ...
    if (block) {
      ...
      var a = block_();
      name && _verify_symbol(name);
      args && args.forEach(_verify_symbol);
      S.input.pop_directives_stack();
    } else a = [new AST_Return({ start: S.token, value: expression(false), end: S.token })];
  ...
  function _await_expression() {
    is_in_async() || croak("Unexpected await expression outside async function", S.prev.line, S.prev.col, S.prev.pos);

    return new AST_Await({ start: prev(), end: S.token, expression: maybe_unary(true) });
  }
  ...
  function _yield_expression() {
    ...
    if (can_insert_semicolon() || (is("punc") && PUNC_AFTER_EXPRESSION.has(S.token.value))) has_expression = false;
    else if (is("operator", "*")) {
      star = true;
      next();
    }

    return new AST_Yield({ start: start, is_star: star, expression: has_expression ? expression() : null, end: prev() });
  }
  ...
  function switch_body_() {
    expect("{");
    var tmp, a = [], cur = null, branch = null;
    while (!is("punc", "}")) {
      is("eof") && unexpected();
      if (is("keyword", "case")) {
  ...
  function vardefs(no_in, kind) {
    ...
    for (;;) {
      ...
      if (is("punc", "{") || is("punc", "["))
        ...
        })).name.name != "import" || croak("Unexpected token: import");
  ...
  var var_ = function(no_in) {
    return new AST_Var({ start: prev(), definitions: vardefs(no_in, "var"), end: prev() });
  };

  var let_ = function(no_in) {
    return new AST_Let({ start: prev(), definitions: vardefs(no_in, "let"), end: prev() });
  };

  var const_ = function(no_in) {
    return new AST_Const({ start: prev(), definitions: vardefs(no_in, "const"), end: prev() });
  };
  ...
  var new_ = function(allow_calls) {
    ...
    if (is("punc", ".")) {
      ...
      return subscripts(new AST_NewTarget({ start: start, end: prev() }), allow_calls);
    }
    var args, newexp = expr_atom(false);
    if (is("punc", "(")) {
      next();
  ...
  function to_fun_args(ex, _, __, default_seen_above) {
    var insert_default = function(ex, default_value) {
      return default_value
        ? new AST_DefaultAssign({ start: ex.start, left: ex, operator: "=", right: default_value, end: default_value.end })
        : ex;
    };
    ...
    if (ex instanceof AST_ObjectKeyVal) {
      ex.value = to_fun_args(ex.value, 0, [ex.key]);
      return insert_default(ex, default_seen_above);
    }
    if (ex instanceof AST_Hole) return ex;
    if (ex instanceof AST_Destructuring) {
      ex.names = ex.names.map(to_fun_args);
      return insert_default(ex, default_seen_above);
    }
    if (ex instanceof AST_SymbolRef)
      return insert_default(new AST_SymbolFunarg({ name: ex.name, start: ex.start, end: ex.end }), default_seen_above);
    if (ex instanceof AST_Expansion) {
      ex.expression = to_fun_args(ex.expression);
      return insert_default(ex, default_seen_above);
    }
    if (ex instanceof AST_Array)
      return insert_default(new AST_Destructuring({
        start: ex.start,
        end: ex.end,
        is_array: true,
    ...
    if (ex instanceof AST_Assign) return insert_default(to_fun_args(ex.left, void 0, void 0, ex.right), default_seen_above);
    if (ex instanceof AST_DefaultAssign) {
  ...
  var expr_atom = function(allow_calls, allow_arrows) {
    ...
    var peeked,
      start = S.token,
      async = is("name", "async") && (peeked = peek()).value != "[" && peeked.type != "arrow" && as_atom_node();
    if (is("punc")) {
      switch (S.token.value) {
      case "(":
        ...
        var ex = async ? new AST_Call({ expression: async, args: exprs })
          : exprs.length == 1 ? exprs[0] : new AST_Sequence({ expressions: exprs });
        if (ex.start) {
        ...
        ex.end = end;
        ex instanceof AST_Call && annotate(ex);
    ...
    if (allow_arrows && is("name") && is_token(peek(), "arrow")) {
      var param = new AST_SymbolFunarg({ name: S.token.value, start: start, end: start });
      next();
    ...
    if (ATOMIC_START_TOKEN.has(S.token.type)) return subscripts(as_atom_node(), allow_calls);
    unexpected();
  };
  ...
  function template_string() {
    ...
    segments.push(new AST_TemplateSegment({ start: S.token, raw: S.token.raw, value: S.token.value, end: S.token }));
    while (!S.token.end) {
      next();
      handle_regexp();
      segments.push(expression(true));

      is_token("template_substitution") || unexpected();

      segments.push(new AST_TemplateSegment({ start: S.token, raw: S.token.raw, value: S.token.value, end: S.token }));
    }
    next();

    return new AST_TemplateString({ start: start, segments: segments, end: S.token });
  }
  ...
  function expr_list(closing, allow_trailing_comma, allow_empty) {
    ...
    while (!is("punc", closing)) {
      ...
      if (is("punc", ",") && allow_empty) a.push(new AST_Hole({ start: S.token, end: S.token }));
      else if (is("expand", "...")) {
        next();
        a.push(new AST_Expansion({ start: prev(), expression: expression(), end: S.token }));
  ...
  var create_accessor = embed_tokens((is_generator, is_async) => function_(AST_Accessor, is_generator, is_async));

  var object_or_destructuring_ = embed_tokens(function() {
    ...
    while (!is("punc", "}")) {
      ...
      if ((start = S.token).type == "expand") {
        next();
        a.push(new AST_Expansion({ start: start, expression: expression(false), end: prev() }));
        continue;
      }
      ...
      if (!is("punc", ":")) {
        ...
        value = new AST_SymbolRef({ start: prev(), name: name, end: prev() });
      } else if (name === null) unexpected(prev());
      else {
        next();
      ...
      if (is("operator", "=")) {
        next();
        value = new AST_Assign({ start: start, left: value, operator: "=", right: expression(false), end: prev() });
      }
  ...
  function class_(KindOfClass) {
    ...
    KindOfClass !== AST_DefClass || class_name || unexpected();
    ...
    while (!is("punc", "}")) {
      start = S.token;
      (method = concise_method_or_getset(as_property_name(), start, true)) || unexpected();
      a.push(method);
      while (is("punc", ";")) next();
    }
    ...
    return new KindOfClass({ start: start, name: class_name, extends: extends_, properties: a, end: prev() });
  }

  function concise_method_or_getset(name, start, is_class) {
    var get_method_name_ast = function(name, start) {
      if (typeof name == "string" || typeof name == "number")
        return new AST_SymbolMethod({ start, name: "" + name, end: prev() });
      name !== null || unexpected();

      return name;
    };
    const get_class_property_key_ast = name => {
      if (typeof name == "string" || typeof name == "number")
        return new AST_SymbolClassProperty({ start: property_token, end: property_token, name: "" + name });
      name !== null || unexpected();
    ...
    if (name === null) {
      is_generator = true;
      property_token = S.token;
      (name = as_property_name()) !== null || unexpected();
    }
    if (is("punc", "(")) {
    ...
    } else if (name == "set" && (!is("punc") || is("punc", "["))) {
      name = get_method_name_ast(as_property_name(), start);
    ...
    if (is_class) {
      const key = get_class_property_key_ast(name),
        quote = key instanceof AST_SymbolClassProperty ? property_token.quote : void 0;
      if (is("operator", "=")) {
        next();
        return new AST_ClassProperty({ start, static: is_static, quote, key, value: expression(false), end: prev() });
      }
      if (is("name") || is("punc", ";") || is("punc", "}"))
        return new AST_ClassProperty({ start, static: is_static, quote, key, end: prev() });
    }
  }
  ...
  function import_() {
    ...
    if ((imported_names = map_names(true)) || imported_name) expect_token("name", "from");

    var mod_str = S.token;
    mod_str.type === "string" || unexpected();
    ...
    return new AST_Import({
      ...
      module_name: new AST_String({ start: mod_str, value: mod_str.value, quote: mod_str.quote, end: mod_str }),
      end: S.token
    });
  }
  ...
  function map_name(is_import) {
    function make_symbol(type) {
      return new type({ name: as_property_name(), start: prev(), end: prev() });
    }

    var foreign_name,
      name,
      foreign_type = is_import ? AST_SymbolImportForeign : AST_SymbolExportForeign,
      type = is_import ? AST_SymbolImport : AST_SymbolExport,
      start = S.token;

    is_import ? (foreign_name = make_symbol(foreign_type)) : (name = make_symbol(type));

    if (is("name", "as")) {
      next();
      is_import ? (name = make_symbol(type)) : (foreign_name = make_symbol(foreign_type));
    } else is_import ? (name = new type(foreign_name)) : (foreign_name = new foreign_type(name));

    return new AST_NameMapping({ start: start, foreign_name: foreign_name, name: name, end: prev() });
  }
  ...
  function map_nameAsterisk(is_import, name) {
    ...
    name = name || new type({ name: "*", start: start, end: end });

    foreign_name = new foreign_type({ name: "*", start: start, end: end });

    return new AST_NameMapping({ start: start, foreign_name: foreign_name, name: name, end: end });
  }
  ...
  function export_() {
    ...
    if (is("keyword", "default")) {
      ...
      return new AST_Export({ start: start, is_default: is_default, exported_names: exported_names, end: prev() });
    }

    if (is("punc", "{") || (is_default && (is("keyword", "class") || is("keyword", "function")) && is_token(peek(), "punc"))) {
      exported_value = expression(false);
      semicolon();
    } else (node = statement(is_default)) instanceof AST_Definitions && is_default
      ? unexpected(node.start)
      : node instanceof AST_Definitions || node instanceof AST_Lambda || node instanceof AST_DefClass
  ...
  function _verify_symbol(sym) {
    var name = sym.name;
    is_in_generator() && name == "yield" && token_error(sym.start, "Yield cannot be used as identifier inside generators");

    if (S.input.has_directive("use strict")) {
      name != "yield" || token_error(sym.start, "Unexpected yield identifier inside strict mode");

      sym instanceof AST_SymbolDeclaration && (name == "arguments" || name == "eval") &&
  ...
  function annotate(node) {
    ...
    for (var i = comments_outside_parens != null ? comments_outside_parens : comments.length; --i >= 0; ) {
  ...
  var subscripts = function(expr, allow_calls) {
    ...
    if (allow_calls && is("punc", "(")) {
      next();
      var call = new AST_Call({ start: start, expression: expr, args: call_args(), end: prev() });
      annotate(call);
  ...
  function call_args() {
    ...
    while (!is("punc", ")")) {
      if (is("expand", "...")) {
        next();
        args.push(new AST_Expansion({ start: prev(), expression: expression(false), end: prev() }));
  ...
  var maybe_unary = function(allow_calls, allow_arrows) {
    ...
    while (is("operator") && UNARY_POSTFIX.has(S.token.value) && !has_newline_before(S.token)) {
      val instanceof AST_Arrow && unexpected();
      (val = make_unary(AST_UnaryPostfix, S.token, val)).start = start;
      val.end = S.token;
      next();
    }
    return val;
  ...
  var expr_op = function(left, min_prec, no_in) {
    ...
    op == "**" && left instanceof AST_UnaryPrefix &&
      !is_token(left.start, "punc", "(") && left.operator !== "--" && left.operator !== "++" &&
  ...
  function to_destructuring(node) {
    ...
    } else if (node instanceof AST_ObjectProperty) node.value = to_destructuring(node.value);
    else if (node instanceof AST_Assign)
      node = new AST_DefaultAssign({ start: node.start, left: node.left, operator: "=", right: node.right, end: node.end });
  ...
  return options.expression ? expression(true) : (function() {
    var start = S.token,
      body = [];
    S.input.push_directives_stack();
    options.module && S.input.add_directive("use strict");
    ...
    if (toplevel) {
      ...
      toplevel.end = end;
    } else toplevel = new AST_Toplevel({ start: start, body: body, end: end });
...
function DEFNODE(type, props, methods, base = AST_Node) {
  ...
  base && base.SUBCLASSES.push(ctor);
  ...
  if (methods) for (i in methods) if (HOP(methods, i))
    i[0] === "$" ? (ctor[i.substr(1)] = methods[i]) : (ctor.prototype[i] = methods[i]);
...
var AST_Block = DEFNODE("Block", "body block_scope", {
  ...
  $propdoc: { body: "[AST_Statement*] an array of statements", block_scope: "[AST_Scope] the block scope" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
  ...
  _children_backwards(push) {
    for (let i = this.body.length; i--; ) push(this.body[i]);
  },
...
var AST_IterationStatement = DEFNODE("IterationStatement", "block_scope", {
  ...
  $propdoc: { block_scope: "[AST_Scope] the block scope for this iteration statement." },
  clone: clone_block_scope
}, AST_StatementWithBody);
...
var AST_For = DEFNODE("For", "init condition step", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.init && this.init._walk(visitor);
      this.condition && this.condition._walk(visitor);
      this.step && this.step._walk(visitor);
      this.body._walk(visitor);
    });
  },
  _children_backwards(push) {
    push(this.body);
    this.step && push(this.step);
    this.condition && push(this.condition);
...
var AST_ForIn = DEFNODE("ForIn", "init object", {
  ...
  $propdoc: { init: "[AST_Node] the `for/in` initialization code", object: "[AST_Node] the object that we're looping through" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
  ...
  _children_backwards(push) {
    push(this.body);
    this.object && push(this.object);
    this.init && push(this.init);
  }
}, AST_IterationStatement);

var AST_ForOf = DEFNODE("ForOf", "await", { $documentation: "A `for ... of` statement" }, AST_ForIn);
...
var AST_Toplevel = DEFNODE("Toplevel", "globals", {
  ...
  wrap_commonjs: function(name) {
    var body = this.body,
      wrapped_tl = "(function(exports){'$ORIG';})(typeof " + name + "=='undefined'?(" + name + "={}):" + name + ");";
    return parse(wrapped_tl).transform(new TreeTransformer(function(node) {
      if (node instanceof AST_Directive && node.value == "$ORIG") return MAP.splice(body);
    }));
  },
  wrap_enclose: function(args_values) {
    ...
    var body = this.body;
    return parse(["(function(", args_values.slice(0, index), '){"$ORIG"})(', args_values.slice(index + 1), ")"].join("")).transform(
...
var AST_Lambda = DEFNODE("Lambda", "name argnames uses_arguments is_generator async", {
  ...
  args_as_names: function() {
    ...
    for (var i = 0; i < this.argnames.length; i++)
      this.argnames[i] instanceof AST_Destructuring ? out.push(...this.argnames[i].all_symbols()) : out.push(this.argnames[i]);
...
var AST_Function = DEFNODE("Function", null, { $documentation: "A function expression" }, AST_Lambda),
  AST_Arrow = DEFNODE("Arrow", null, { $documentation: "An ES6 Arrow function ((a) => b)" }, AST_Lambda),
...
var AST_Destructuring = DEFNODE("Destructuring", "names is_array", {
  ...
  _children_backwards(push) {
    for (let i = this.names.length; i--; ) push(this.names[i]);
  },
...
var AST_TemplateString = DEFNODE("TemplateString", "segments", {
  ...
  _children_backwards(push) {
    for (let i = this.segments.length; i--; ) push(this.segments[i]);
  }
});

var AST_TemplateSegment = DEFNODE("TemplateSegment", "value raw", {
  $documentation: "A segment of a template string literal",
  $propdoc: { value: "Content of the segment", raw: "Raw content of the segment" }
});
...
var AST_Await = DEFNODE("Await", "expression", {
  ...
  $propdoc: { expression: "[AST_Node] the mandatory expression being awaited" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.expression._walk(visitor);
    });
...
var AST_Yield = DEFNODE("Yield", "expression is_star", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, this.expression && (() => {
      this.expression._walk(visitor);
    }));
  },
  _children_backwards(push) {
...
var AST_If = DEFNODE("If", "condition alternative", {
  ...
  $propdoc: { condition: "[AST_Node] the `if` condition", alternative: "[AST_Statement?] the `else` part, or null if not present" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.condition._walk(visitor);
      this.body._walk(visitor);
      this.alternative && this.alternative._walk(visitor);
    });
  },
  _children_backwards(push) {
    this.alternative && push(this.alternative);
...
var AST_Try = DEFNODE("Try", "bcatch bfinally", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      walk_body(this, visitor);
      this.bcatch && this.bcatch._walk(visitor);
      this.bfinally && this.bfinally._walk(visitor);
    });
  },
  _children_backwards(push) {
    this.bfinally && push(this.bfinally);
    this.bcatch && push(this.bcatch);
    for (let i = this.body.length; i--; ) push(this.body[i]);
  }
}, AST_Block);
...
var AST_Catch = DEFNODE("Catch", "argname", {
  ...
  $propdoc: { argname: "[AST_SymbolCatch|AST_Destructuring|AST_Expansion|AST_DefaultAssign] symbol for the exception" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.argname && this.argname._walk(visitor);
      walk_body(this, visitor);
    });
  },
  _children_backwards(push) {
...
var AST_Definitions = DEFNODE("Definitions", "definitions", {
  ...
  _children_backwards(push) {
    for (let i = this.definitions.length; i--; ) push(this.definitions[i]);
  }
}, AST_Statement);

var AST_Var = DEFNODE("Var", null, { $documentation: "A `var` statement" }, AST_Definitions),
  AST_Let = DEFNODE("Let", null, { $documentation: "A `let` statement" }, AST_Definitions),
...
var AST_Import = DEFNODE("Import", "imported_name imported_names module_name", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.imported_name && this.imported_name._walk(visitor);
  ...
  _children_backwards(push) {
    ...
    if (this.imported_names) for (let i = this.imported_names.length; i--; ) push(this.imported_names[i]);
...
var AST_Export = DEFNODE("Export", "exported_definition exported_value is_default exported_names module_name", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.exported_definition && this.exported_definition._walk(visitor);
      this.exported_value && this.exported_value._walk(visitor);
  ...
  _children_backwards(push) {
    this.module_name && push(this.module_name);
    if (this.exported_names) for (let i = this.exported_names.length; i--; ) push(this.exported_names[i]);

    this.exported_value && push(this.exported_value);
    this.exported_definition && push(this.exported_definition);
  }
}, AST_Statement);
...
var AST_Call = DEFNODE("Call", "expression args _annotations", {
  ...
  _children_backwards(push) {
    for (let i = this.args.length; i--; ) push(this.args[i]);
    push(this.expression);
  }
});
...
var AST_Sequence = DEFNODE("Sequence", "expressions", {
  ...
  _children_backwards(push) {
    for (let i = this.expressions.length; i--; ) push(this.expressions[i]);
  }
});
...
var AST_Dot = DEFNODE("Dot", "quote", {
  ...
  $propdoc: { quote: "[string] the original quote character when transformed from AST_Sub" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.expression._walk(visitor);
    });
...
var AST_Unary = DEFNODE("Unary", "operator expression", {
  ...
  $propdoc: { operator: "[string] the operator", expression: "[AST_Node] expression that this unary operator applies to" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.expression._walk(visitor);
    });
...
var AST_Array = DEFNODE("Array", "elements", {
  ...
  _children_backwards(push) {
    for (let i = this.elements.length; i--; ) push(this.elements[i]);
  }
});
...
var AST_Object = DEFNODE("Object", "properties", {
  ...
  _children_backwards(push) {
    for (let i = this.properties.length; i--; ) push(this.properties[i]);
  }
});
...
var AST_ObjectProperty = DEFNODE("ObjectProperty", "key value", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.key instanceof AST_Node && this.key._walk(visitor);
      this.value._walk(visitor);
    });
  },
  _children_backwards(push) {
    push(this.value);
    this.key instanceof AST_Node && push(this.key);
  }
});
...
var AST_Class = DEFNODE("Class", "name extends properties", {
  ...
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.name && this.name._walk(visitor);
      this.extends && this.extends._walk(visitor);
  ...
  _children_backwards(push) {
    for (let i = this.properties.length; i--; ) push(this.properties[i]);
    this.extends && push(this.extends);
    this.name && push(this.name);
  }
}, AST_Scope);
...
var AST_ClassProperty = DEFNODE("ClassProperty", "static quote", {
  ...
  $propdoc: { static: "[boolean] whether this is a static key", quote: "[string] which quote is being used" },
  _walk: function(visitor) {
    return visitor._visit(this, function() {
      this.key instanceof AST_Node && this.key._walk(visitor);
      this.value instanceof AST_Node && this.value._walk(visitor);
    });
  },
  _children_backwards(push) {
    this.value instanceof AST_Node && push(this.value);
    this.key instanceof AST_Node && push(this.key);
  },
  computed_key() {
...
var AST_DefClass = DEFNODE("DefClass", null, { $documentation: "A class definition" }, AST_Class),
  AST_ClassExpression = DEFNODE("ClassExpression", null, { $documentation: "A class expression." }, AST_Class);
...
var AST_NewTarget = DEFNODE("NewTarget", null, { $documentation: "A reference to new.target" });
...
var AST_SymbolConst = DEFNODE("SymbolConst", null, { $documentation: "A constant declaration" }, AST_SymbolBlockDeclaration),
  AST_SymbolLet = DEFNODE("SymbolLet", null, { $documentation: "A block-scoped `let` declaration" }, AST_SymbolBlockDeclaration),
  ...
  AST_SymbolMethod = DEFNODE("SymbolMethod", null, { $documentation: "Symbol in an object defining a method" }, AST_Symbol),
  AST_SymbolClassProperty = DEFNODE("SymbolClassProperty", null, { $documentation: "Symbol for a class property" }, AST_Symbol),
...
var AST_SymbolExport = DEFNODE("SymbolExport", null, { $documentation: "Symbol referring to a name to export" }, AST_SymbolRef);
...
var AST_Atom = DEFNODE("Atom", null, { $documentation: "Base class for atoms" }, AST_Constant),
  ...
  AST_NaN = DEFNODE("NaN", null, { $documentation: "The impossible value", value: 0/0 }, AST_Atom),
  AST_Undefined = DEFNODE("Undefined", null, { $documentation: "The `undefined` value", value: void 0 }, AST_Atom),
  AST_Hole = DEFNODE("Hole", null, { $documentation: "A hole in an array", value: void 0 }, AST_Atom),
  AST_Infinity = DEFNODE("Infinity", null, { $documentation: "The `Infinity` value", value: 1/0 }, AST_Atom),
  AST_Boolean = DEFNODE("Boolean", null, { $documentation: "Base class for booleans" }, AST_Atom),
  AST_False = DEFNODE("False", null, { $documentation: "The `false` atom", value: false }, AST_Boolean),
  AST_True = DEFNODE("True", null, { $documentation: "The `true` atom", value: true }, AST_Boolean);

function walk(node, cb, to_visit = [node]) {
  const push = to_visit.push.bind(to_visit);
  while (to_visit.length) {
    const node = to_visit.pop(),
      ret = cb(node, to_visit);
...
function walk_parent(node, cb, initial_stack) {
  const to_visit = [node],
    push = to_visit.push.bind(to_visit),
    stack = initial_stack ? initial_stack.slice() : [],
    parent_pop_indices = [];
...
class TreeWalker {
  ...
  push(node) {
    if (node instanceof AST_Lambda) this.directives = Object.create(this.directives);
    else if (node instanceof AST_Directive && !this.directives[node.value]) this.directives[node.value] = node;
    else if (node instanceof AST_Class) {
      this.directives = Object.create(this.directives);
      this.directives["use strict"] || (this.directives["use strict"] = node);
    }
...
def_transform(AST_Import, function(self, tw) {
  ...
  self.imported_names && do_list(self.imported_names, tw);
...
def_transform(AST_Export, function(self, tw) {
  ...
  self.exported_names && do_list(self.exported_names, tw);
...
function left_is_object(node) {
  return node instanceof AST_Object || (
    node instanceof AST_Sequence ? left_is_object(node.expressions[0])
    : node.TYPE === "Call" ? left_is_object(node.expression)
    : node instanceof AST_PrefixedTemplateString ? left_is_object(node.prefix)
    : node instanceof AST_Dot || node instanceof AST_Sub ? left_is_object(node.expression)
    : node instanceof AST_Conditional ? left_is_object(node.condition)
    : node instanceof AST_Binary ? left_is_object(node.left)
    : node instanceof AST_UnaryPostfix && left_is_object(node.expression)
  );
}
...
function OutputStream(options) {
  ...
  if (options.comments) {
    ...
    comment_filter = comments instanceof RegExp
      ? function(comment) {
  ...
  function make_string(str, quote) {
    ...
    str = str.replace(/[\\\b\f\n\r\v\t\x22\x27\u2028\u2029\0\ufeff]/g, function(s, i) {
      ...
      case "\b": return "\\b";
      case "\f": return "\\f";
      case "\v": return options.ie8 ? "\\x0B" : "\\v";
      case "\u2028": return "\\u2028";
      case "\u2029": return "\\u2029";
      case "\ufeff": return "\\ufeff";
      case "\0":
  ...
  var mapping_token, mapping_name, has_parens = false,
    ...
    if (current_col > options.max_line_len) {
      if (might_add_newline) {
        var left = OUTPUT.slice(0, might_add_newline),
          right = OUTPUT.slice(might_add_newline);
        if (mappings) {
  ...
  function print(str) {
    ...
    if (might_need_semicolon) {
      ...
      if ((prev === ":" && ch === "}") || ((!ch || !";}".includes(ch)) && prev !== ";")) {
        ...
        options.beautify || (might_need_space = false);
      }
    }
    ...
    if (mapping_token) {
      ...
      mapping_token = false;
      might_add_newline || do_add_mapping();
    }
  ...
  var indent = options.beautify ? function(half) {
    options.beautify && print(make_indent(half ? 0.5 : 0));
  } : noop;
  ...
  function get() {
    might_add_newline && ensure_line_len();

    return OUTPUT;
  }

  function has_nlb() {
    for (let n = OUTPUT.length - 1; n >= 0; ) {
  ...
  function filter_comment(comment) {
    options.preserve_annotations || (comment = comment.replace(r_annotation, " "));

    return /^\s*$/.test(comment) ? "" : comment.replace(/(<\s*\/\s*)(script)/i, "<\\/$2");
  }
  ...
  function prepend_comments(node) {
    ...
    comments || (comments = start.comments_before = []);
    ...
    if ((comments = comments.filter(comment_filter, node).filter(c => !printed_comments.has(c))).length == 0) return;
    ...
    comments.forEach(function(c, i) {
      ...
      if (/comment[134]/.test(c.type)) {
        if ((value = filter_comment(c.value))) {
          print("//" + value + "\n");
          indent();
        }
        last_nlb = true;
      } else if (c.type == "comment2") {
        (value = filter_comment(c.value)) && print("/*" + value + "*/");
  ...
  function append_comments(node, tail) {
    ...
    if (!token) return;
    var printed_comments = self.printed_comments,
      comments = token[tail ? "comments_before" : "comments_after"];
    if (!comments || printed_comments.has(comments) ||
        !(node instanceof AST_Statement || comments.every(c => !/comment[134]/.test(c.type)))) return;
    printed_comments.add(comments);
  ...
  return {
    ...
    current_width: () => current_col - indentation,
    ...
    print_string: function(str, quote, escape_directive) {
      ...
      if (escape_directive === true && !encoded.includes("\\")) {
        EXPECT_DIRECTIVE.test(OUTPUT) || force_semicolon();
    ...
    pop_node: () => stack.pop(),
    parent: (n) => stack[stack.length - 2 - (n || 0)]
  };
}
...
!(function() {
  ...
  AST_Node.DEFMETHOD("print", function(output, force_parens) {
    ...
    else if (!output.use_asm && self instanceof AST_Directive && self.value == "use asm") output.use_asm = output.active_scope;
    ...
    force_parens || self.needs_parens(output) ? output.with_parens(doit) : doit();
  ...
  PARENS(AST_Function, function(output) {
    ...
    var p;
    if (output.option("webkit") && (p = output.parent()) instanceof AST_PropAccess && p.expression === this) return true;

    if (output.option("wrap_iife") && (p = output.parent()) instanceof AST_Call && p.expression === this) return true;

    if (output.option("wrap_func_args") && (p = output.parent()) instanceof AST_Call && p.args.includes(this)) return true;
  ...
  PARENS(AST_Binary, function(output) {
    ...
    if ((p instanceof AST_Call && p.expression === this) ||
        p instanceof AST_Unary ||
        (p instanceof AST_PropAccess && p.expression === this)) return true;
    if (p instanceof AST_Binary) {
      const po = p.operator,
        so = this.operator;

      if (so === "??" && (po === "||" || po === "&&")) return true;
  ...
  PARENS(AST_New, function(output) {
    var p = output.parent();
    if (this.args.length === 0 && (p instanceof AST_PropAccess || (p instanceof AST_Call && p.expression === this)))
      return true;
  });
  ...
  PARENS(AST_BigInt, function(output) {
    var p = output.parent();
    if (p instanceof AST_PropAccess && p.expression === this && this.getValue().startsWith("-")) return true;
  });
  ...
  PARENS([AST_Assign, AST_Conditional], function(output) {
    ...
    if (
      p instanceof AST_Unary ||
      (p instanceof AST_Binary && !(p instanceof AST_Assign)) ||
      (p instanceof AST_Call && p.expression === this) ||
      (p instanceof AST_Conditional && p.condition === this) ||
      (p instanceof AST_PropAccess && p.expression === this) ||
      (this instanceof AST_Assign && this.left instanceof AST_Destructuring && this.left.is_array === false)
    )
      return true;
  });
  ...
  DEFPRINT(AST_Destructuring, function(self, output) {
    ...
    self.names.forEach(function(name, i) {
      i > 0 && output.comma();
      name.print(output);
      i == len - 1 && name instanceof AST_Hole && output.comma();
    });
    output.print(self.is_array ? "]" : "}");
  });
  ...
  DEFPRINT(AST_For, function(self, output) {
    ...
    output.with_parens(function() {
      if (self.init) {
        self.init instanceof AST_Definitions ? self.init.print(output) : parenthesize_for_noin(self.init, output, true);
      ...
      self.step && self.step.print(output);
    });
    output.space();
    self._do_print_body(output);
  });
  ...
  AST_Lambda.DEFMETHOD("_do_print", function(output, nokeyword) {
    ...
    if (!nokeyword) {
      ...
      self.is_generator && output.star();

      self.name && output.space();
    }
    self.name instanceof AST_Symbol ? self.name.print(output)
      : nokeyword && self.name instanceof AST_Node &&
  ...
  DEFPRINT(AST_TemplateString, function(self, output) {
    ...
    for (var i = 0; i < self.segments.length; i++)
      if (!(self.segments[i] instanceof AST_TemplateSegment)) {
        ...
        output.print("}");
      } else is_tagged ? output.print(self.segments[i].raw) : output.print_template_string_chars(self.segments[i].value);
  ...
  AST_Arrow.DEFMETHOD("_do_print", function(output) {
    ...
    self.argnames.length === 1 && self.argnames[0] instanceof AST_Symbol ? self.argnames[0].print(output)
      : output.with_parens(function() {
          self.argnames.forEach(function(arg, i) {
            i && output.comma();
            arg.print(output);
    ...
    if (self.body.length === 1 && first_statement instanceof AST_Return) {
      ...
      if (!returned) output.print("{}");
      else if (left_is_object(returned)) {
        output.print("(");
        returned.print(output);
        output.print(")");
      } else returned.print(output);
  ...
  DEFPRINT(AST_If, function(self, output) {
    ...
    if (self.alternative) {
      ...
      output.print("else");
      output.space();
      self.alternative instanceof AST_If ? self.alternative.print(output) : force_statement(self.alternative, output);
  ...
  DEFPRINT(AST_Switch, function(self, output) {
    ...
    last < 0 ? print_braced_empty(self, output) : output.with_block(function() {
      self.body.forEach(function(branch, i) {
        output.indent(true);
        branch.print(output);
        i < last && branch.body.length > 0 && output.newline();
      });
    });
  });
  ...
  AST_Definitions.DEFMETHOD("_do_print", function(output, kind) {
    ...
    this.definitions.forEach(function(def, i) {
      if (i) output.print(","), output.should_break() && (output.newline(), output.indent()), output.space();
      def.print(output);
    });
    var p = output.parent();
    if (!(p instanceof AST_For || p instanceof AST_ForIn) || (p && p.init !== this)) output.semicolon();
  });
  ...
  DEFPRINT(AST_Import, function(self, output) {
    output.print("import");
    output.space();
    self.imported_name && self.imported_name.print(output);
    ...
    if (self.imported_names)
      if (self.imported_names.length === 1 && self.imported_names[0].foreign_name.name === "*")
        ...
        self.imported_names.forEach(function(name_import, i) {
          ...
          i < self.imported_names.length - 1 && output.print(",");
        });
        output.space();
        output.print("}");
      }
  ...
  DEFPRINT(AST_NameMapping, function(self, output) {
    ...
    if (((definition && definition.mangled_name) || self.name.name) !== self.foreign_name.name) {
      is_import ? output.print(self.foreign_name.name) : self.name.print(output);

      output.space();
      output.print("as");
      output.space();
      is_import ? self.name.print(output) : output.print(self.foreign_name.name);
  ...
  DEFPRINT(AST_Export, function(self, output) {
    ...
    if (self.exported_names)
      if (self.exported_names.length === 1 && self.exported_names[0].name.name === "*") self.exported_names[0].print(output);
      else {
        output.print("{");
        self.exported_names.forEach(function(name_export, i) {
          output.space();
          name_export.print(output);
          i < self.exported_names.length - 1 && output.print(",");
        });
        output.space();
        output.print("}");
      }
    else if (self.exported_value) self.exported_value.print(output);
  ...
  function parenthesize_for_noin(node, output, noin) {
    ...
    if (noin)
      parens = walk(node, node =>
        node instanceof AST_Scope || (node instanceof AST_Binary && node.operator == "in" ? walk_abort : void 0));
  ...
  AST_Sequence.DEFMETHOD("_do_print", function(output) {
    this.expressions.forEach(function(node, index) {
      if (index > 0) {
        output.print(",");
        if (!output.should_break()) output.space();
        else {
          output.newline();
          output.indent();
        }
  ...
  DEFPRINT(AST_Dot, function(self, output) {
    ...
    if (RESERVED_WORDS.has(prop) ? output.option("ie8") : !is_identifier_string(prop, output.option("ecma") >= 2015)) {
      output.print("[");
      output.add_mapping(self.end);
      output.print_string(prop);
      output.print("]");
    } else {
      expr instanceof AST_Number && expr.getValue() >= 0 && !/[xa-f.)]/i.test(output.last()) && output.print(".");
  ...
  DEFPRINT(AST_UnaryPrefix, function(self, output) {
    ...
    output.print(op);
    if (/^[a-z]/i.test(op) || (/[+-]$/.test(op) &&
        self.expression instanceof AST_UnaryPrefix && /^[+-]/.test(self.expression.operator)))
      output.space();
  ...
  DEFPRINT(AST_Binary, function(self, output) {
    ...
    op[0] == ">" && self.left instanceof AST_UnaryPostfix && self.left.operator == "--" ? output.print(" ") : output.space();
  ...
  DEFPRINT(AST_Array, function(self, output) {
    output.with_square(function() {
      ...
      a.forEach(function(exp, i) {
        i && output.comma();
        exp.print(output);
        i === len - 1 && exp instanceof AST_Hole && output.comma();
      });
      len > 0 && output.space();
    });
  });
  ...
  function print_property_name(key, quote, output, tok) {
    return output.option("quote_keys") ? output.print_string(key)

      : "" + +key == key && key >= 0
      ? output.option("keep_numbers")
        ? output.print(output.option("keep_numbers") === true && tok && tok.raw != null ? tok.raw : key)
      ...
      : (RESERVED_WORDS.has(key)
        ...
        (quote && output.option("keep_quoted_props")) ? output.print_string(key, quote) : output.print_name(key);
  }
  ...
  DEFPRINT(AST_ClassProperty, (self, output) => {
    ...
    if (self.key instanceof AST_SymbolClassProperty) print_property_name(self.key.name, self.quote, output);
    else {
      output.print("[");
      self.key.print(output);
      output.print("]");
  ...
  DEFPRINT(AST_ConciseMethod, function(self, output) {
    var type;
    if (self.is_generator && self.async) type = "async*";
    else if (self.is_generator) type = "*";
    else if (self.async) type = "async";
  ...
  DEFPRINT(AST_Number, function(self, output) {
    (output.option("keep_numbers") || output.use_asm) && self.start && self.start.raw != null
  ...
  const r_slash_script = /(<\s*\/\s*script)/i,
    slash_script_replace = (_, $1) => $1.replace("/", "\\/");
  ...
  function force_statement(stat, output) {
    output.option("braces")
      ...
      : !stat || stat instanceof AST_EmptyStatement
  ...
  function best_of(a) {
    var best = a[0];
    for (var len = best.length, i = 1; i < a.length; ++i) if (a[i].length < len) len = (best = a[i]).length;
  ...
  function make_num(/** number */ num) {
    ...
    if (Math.floor(num) === num)
      num < 0 ? candidates.push("-0x" + (-num).toString(16).toLowerCase()) : candidates.push("0x" + num.toString(16).toLowerCase());
    ...
    } else (match = /^(\d)\.(\d+)e(-?\d+)$/.exec(str)) && candidates.push(match[1] + match[2] + "e" + (match[3] - match[2].length));

    return best_of(candidates);
  }

  function make_block(stmt, output) {
    !stmt || stmt instanceof AST_EmptyStatement ? output.print("{}")
      : stmt instanceof AST_BlockStatement ? stmt.print(output)
      : output.with_block(function() {
          output.indent();
          stmt.print(output);
          output.newline();
...
const shallow_cmp = (node1, node2) => (node1 === null && node2 === null) || (node1.TYPE === node2.TYPE && node1.shallow_cmp(node2));
...
const equivalent_to = (tree1, tree2) => {
  ...
  const walk_1_state = [tree1],
    ...
    walk_1_push = walk_1_state.push.bind(walk_1_state),
    walk_2_push = walk_2_state.push.bind(walk_2_state);

  while (walk_1_state.length && walk_2_state.length) {
    const node_1 = walk_1_state.pop(),
      node_2 = walk_2_state.pop();
    ...
    if (walk_1_state.length !== walk_2_state.length) return false;
  }
...
const mkshallow = props => {
  const comparisons = Object.keys(props)
    .map(key => {
      if (props[key] === "eq") return `this.${key} === other.${key}`;
      if (props[key] === "exist") return `(this.${key} == null ? other.${key} == null : this.${key} === other.${key})`;
...
AST_For.prototype.shallow_cmp = mkshallow({ init: "exist", condition: "exist", step: "exist" });
...
AST_Lambda.prototype.shallow_cmp = mkshallow({ is_generator: "eq", async: "eq" });
AST_Destructuring.prototype.shallow_cmp = mkshallow({ is_array: "eq" });

AST_PrefixedTemplateString.prototype.shallow_cmp = pass_through;
AST_TemplateString.prototype.shallow_cmp = pass_through;

AST_TemplateSegment.prototype.shallow_cmp = mkshallow({ value: "eq" });

AST_Jump.prototype.shallow_cmp = pass_through;
AST_LoopControl.prototype.shallow_cmp = pass_through;
AST_Await.prototype.shallow_cmp = pass_through;

AST_Yield.prototype.shallow_cmp = mkshallow({ is_star: "eq" });
AST_If.prototype.shallow_cmp = mkshallow({ alternative: "exist" });

AST_Switch.prototype.shallow_cmp = pass_through;
AST_SwitchBranch.prototype.shallow_cmp = pass_through;

AST_Try.prototype.shallow_cmp = mkshallow({ bcatch: "exist", bfinally: "exist" });
AST_Catch.prototype.shallow_cmp = mkshallow({ argname: "exist" });

AST_Finally.prototype.shallow_cmp = pass_through;
AST_Definitions.prototype.shallow_cmp = pass_through;

AST_VarDef.prototype.shallow_cmp = mkshallow({ value: "exist" });

AST_NameMapping.prototype.shallow_cmp = pass_through;

AST_Import.prototype.shallow_cmp = mkshallow({ imported_name: "exist", imported_names: "exist" });
...
AST_Dot.prototype.shallow_cmp = mkshallow({ property: "eq" });
AST_Unary.prototype.shallow_cmp = mkshallow({ operator: "eq" });
AST_Binary.prototype.shallow_cmp = mkshallow({ operator: "eq" });
...
AST_ObjectKeyVal.prototype.shallow_cmp = mkshallow({ key: "eq" });
AST_ObjectSetter.prototype.shallow_cmp = mkshallow({ static: "eq" });
AST_ObjectGetter.prototype.shallow_cmp = mkshallow({ static: "eq" });
AST_ConciseMethod.prototype.shallow_cmp = mkshallow({ static: "eq", is_generator: "eq", async: "eq" });
AST_Class.prototype.shallow_cmp = mkshallow({ name: "exist", extends: "exist" });
AST_ClassProperty.prototype.shallow_cmp = mkshallow({ static: "eq" });
AST_Symbol.prototype.shallow_cmp = mkshallow({ name: "eq" });

AST_NewTarget.prototype.shallow_cmp = pass_through;
AST_This.prototype.shallow_cmp = pass_through;
AST_Super.prototype.shallow_cmp = pass_through;

AST_String.prototype.shallow_cmp = mkshallow({ value: "eq" });
AST_Number.prototype.shallow_cmp = mkshallow({ value: "eq" });
AST_BigInt.prototype.shallow_cmp = mkshallow({ value: "eq" });
...
const MASK_EXPORT_DONT_MANGLE = 1,
  MASK_EXPORT_WANT_MANGLE = 2;
...
class SymbolDef {
  ...
  unmangleable(options) {
    ...
    return (
      !!(function_defs && function_defs.has(this.id) && keep_name(options.keep_fnames, this.orig[0].name)) ||
      (this.global && !options.toplevel) ||
      this.export & MASK_EXPORT_DONT_MANGLE ||
      this.undeclared ||
      (!options.eval && this.scope.pinned()) ||
      ((this.orig[0] instanceof AST_SymbolLambda || this.orig[0] instanceof AST_SymbolDefun) &&
        keep_name(options.keep_fnames, this.orig[0].name)) ||
      this.orig[0] instanceof AST_SymbolMethod ||
      ((this.orig[0] instanceof AST_SymbolClass || this.orig[0] instanceof AST_SymbolDefClass) &&
...
AST_Scope.DEFMETHOD("figure_out_scope", function(options, { parent_scope = null, toplevel = this } = {}) {
  ...
  if (!(toplevel instanceof AST_Toplevel)) throw new Error("Invalid toplevel scope");
  ...
  var tw = new TreeWalker((node, descend) => {
    if (node.is_block_scope()) {
      ...
      options.safari10 && (node instanceof AST_For || node instanceof AST_ForIn) && for_scopes.push(scope);
    ...
    if (node instanceof AST_SymbolLambda) defun.def_function(node, node.name == "arguments" ? void 0 : defun);
      ...
      var def = node instanceof AST_SymbolBlockDeclaration
        ? scope.def_variable(node, null)
        : defun.def_variable(node, node.TYPE == "SymbolVar" ? null : void 0);
      ...
      node instanceof AST_SymbolFunarg || mark_export(def, 2);
      if (defun !== scope) {
        node.mark_enclosed();
        def = scope.find_variable(node);
        if (node.thedef !== def) {
          node.thedef = def;
          node.reference();
        }
      ...
      if (!sym) throw new Error(
        string_template("Undefined label {name} [{line},{col}]", { name: node.name, line: node.start.line, col: node.start.col })
      );
      node.thedef = sym;
    }
    scope instanceof AST_Toplevel || !(node instanceof AST_Export || node instanceof AST_Import) ||
      js_error(
        `"${node.TYPE}" statement may only appear at the top level`, node.start.file, node.start.line, node.start.col, node.start.pos
      );
  });
  this.walk(tw);
  ...
  tw = new TreeWalker(node => {
    ...
    if (node instanceof AST_SymbolRef) {
      ...
      } else if (sym.scope instanceof AST_Lambda && name == "arguments") sym.scope.uses_arguments = true;

      node.thedef = sym;
      node.reference();
      !node.scope.is_block_scope() || sym.orig[0] instanceof AST_SymbolBlockDeclaration ||
  ...
  if (options.ie8 || options.safari10)
    walk(this, node => {
      if (node instanceof AST_SymbolCatch) {
        var name = node.name,
          refs = node.thedef.references,
          scope = node.scope.get_defun_scope(),
          def = scope.find_variable(name) || toplevel.globals.get(name) || scope.def_variable(node);
...
AST_Scope.DEFMETHOD("var_names", function varNames() {
  ...
  if (!var_names) {
    this._var_name_cache = var_names = new Set(this.parent_scope ? varNames.call(this.parent_scope) : null);
    this._added_var_names && this._added_var_names.forEach(name => { var_names.add(name); });
...
AST_Scope.DEFMETHOD("add_var_name", function(name) {
  ...
  this._var_name_cache || this.var_names();
...
AST_Scope.DEFMETHOD("add_child_scope", function(scope) {
  ...
  scope._added_var_names && scope._added_var_names.forEach(name => scope.add_var_name(name));
...
function next_mangled(scope, options) {
  ...
  out: while (1) {
    ...
    if (RESERVED_WORDS.has(m) || options.reserved.has(m) || (unmangleable_names && unmangleable_names.has(m))) continue;
...
AST_Function.DEFMETHOD("next_mangled", function(options, def) {
  var tricky_def = def.orig[0] instanceof AST_SymbolFunarg && this.name && this.name.definition(),
    ...
    tricky_name = tricky_def ? tricky_def.mangled_name || tricky_def.name : null;
...
AST_Toplevel.DEFMETHOD("_default_mangler_options", function(options) {
  ...
  Array.isArray(options.reserved) || options.reserved instanceof Set || (options.reserved = []);
...
AST_Toplevel.DEFMETHOD("mangle_names", function(options) {
  ...
  var tw = new TreeWalker(function(node, descend) {
    ...
    if (node instanceof AST_Scope) node.variables.forEach(collect);
    else if (node.is_block_scope()) node.block_scope.variables.forEach(collect);
    else if (
      function_defs &&
  ...
  function collect(symbol) {
    options.reserved.has(symbol.name) || symbol.export & MASK_EXPORT_DONT_MANGLE || to_mangle.push(symbol);
  }
});
...
AST_Toplevel.DEFMETHOD("find_colliding_names", function(options) {
  ...
  this.walk(new TreeWalker(function(node) {
    node instanceof AST_Scope && node.variables.forEach(add_def);
    node instanceof AST_SymbolCatch && add_def(node.definition());
  }));
  return avoid;
...
AST_Toplevel.DEFMETHOD("expand_names", function(options) {
  ...
  this.walk(new TreeWalker(function(node) {
    node instanceof AST_Scope && node.variables.forEach(rename);
    node instanceof AST_SymbolCatch && rename(node.definition());
  }));
  ...
  function rename(def) {
    ...
    const redefinition = redefined_catch_def(def),
      name = (def.name = redefinition ? redefinition.name : next_name());
...
AST_Toplevel.DEFMETHOD("compute_char_frequency", function(options) {
  ...
  try {
    ...
    AST_Node.prototype.print = function(stream, force_parens) {
      ...
      this instanceof AST_Symbol && !this.unmangleable(options) ? base54.consider(this.name, -1)
        : options.properties &&
          (this instanceof AST_Dot ? base54.consider(this.property, -1) : this instanceof AST_Sub && skip_string(this.property));
    };
  ...
  function skip_string(node) {
    if (node instanceof AST_String) base54.consider(node.value, -1);
    else if (node instanceof AST_Conditional) {
      skip_string(node.consequent);
      skip_string(node.alternative);
    } else node instanceof AST_Sequence && skip_string(node.tail_node());
  }
});
...
function default_options() {
  ...
  Object.keys(infer_options({ 0: 0 })).forEach(component => {
    const options = infer_options({ [component]: { 0: 0 } });
...
AST_Function.prototype._size = function(info) {
  return !!first_in_statement(info) * 2 + lambda_modifiers(this) + 12 + list_overhead(this.argnames) + list_overhead(this.body);
};
...
AST_Arrow.prototype._size = function() {
  ...
  (this.argnames.length === 1 && this.argnames[0] instanceof AST_Symbol) || (args_and_arrow += 2);
...
AST_Import.prototype._size = function() {
  ...
  if (this.imported_names) size += 2 + list_overhead(this.imported_names);
...
AST_Export.prototype._size = function() {
  ...
  if (this.exported_names) size += 2 + list_overhead(this.exported_names);
...
AST_Object.prototype._size = function(info) {
  let base = 2;
  if (first_in_statement(info)) base += 2;
...
AST_SymbolRef.prototype._size = function() {
  ...
  return thedef && thedef.global ? name.length : name === "arguments" ? 9 : 2;
};
...
const UNUSED = 0b00000001,
  ...
  has_flag = (node, flag) => node.flags & flag,
  set_flag = (node, flag) => { node.flags |= flag; },
  clear_flag = (node, flag) => { node.flags &= ~flag; };

class Compressor extends TreeWalker {
  constructor(options, false_by_default) {
    super();
    options.defaults === void 0 || options.defaults || (false_by_default = true);
    ...
    if (typeof global_defs == "object") for (var key in global_defs)
      if (key[0] === "@" && HOP(global_defs, key)) global_defs[key.slice(1)] = parse(global_defs[key], { expression: true });

    if (this.options.inline === true) this.options.inline = 3;
    var pure_funcs = this.options.pure_funcs;
    this.pure_funcs = typeof pure_funcs == "function"
      ? pure_funcs
    ...
    else if (typeof top_retain == "function") this.top_retain = top_retain;
    else if (top_retain) {
  ...
  in_boolean_context() {
    ...
    for (var p, self = this.self(), i = 0; (p = this.parent(i)); i++) {
      ...
      if (
        (p instanceof AST_Binary && (p.operator == "&&" || p.operator == "||" || p.operator == "??")) ||
        p instanceof AST_Conditional ||
        p.tail_node() === self
      )
        self = p;
  ...
  compress(toplevel) {
    ...
    this.option("expression") && toplevel.process_expression(true);

    var passes = +this.options.passes || 1,
      min_count = 1 / 0,
      stopping = false,
      mangle = { ie8: this.option("ie8") };
    for (var pass = 0; pass < passes; pass++) {
      toplevel.figure_out_scope(mangle);
      if (pass === 0 && this.option("drop_console")) toplevel = toplevel.drop_console();

      if (pass > 0 || this.option("reduce_vars")) toplevel.reset_opt_flags(this);
    ...
    this.option("expression") && toplevel.process_expression(false);

    return toplevel;
  }

  info(...args) {
    this.options.warnings != "verbose" || AST_Node.warn(...args);
  }
  ...
  before(node, descend) {
    ...
    var was_scope = false;
    if (node instanceof AST_Scope) {
      node = node.hoist_properties(this).hoist_declarations(this);
      was_scope = true;
    }
    descend(node, this);
    descend(node, this);
    ...
    opt !== node || set_flag(opt, SQUEEZED);
    return opt;
  }
}
...
function read_property(obj, key) {
  if ((key = get_value(key)) instanceof AST_Node) return;
  var value;
...
function is_modified(compressor, tw, node, value, level, immutable) {
  ...
  if (
    ...
    (!(value instanceof AST_Function) || (!(parent instanceof AST_New) && value.contains_this()))
  )
    return true;
...
!(function(def_reduce_vars) {
  ...
  function reset_def(compressor, def) {
    ...
    def.scope.pinned() ? (def.fixed = false)
      : def.orig[0] instanceof AST_SymbolConst || !compressor.exposed(def)
      ? (def.fixed = def.init) : (def.fixed = false);
  }
  ...
  function safe_to_assign(tw, def, scope, value) {
    ...
    return !!HOP(tw.safe_ids, def.id) &&
      ...
      (def.fixed instanceof AST_Defun
        ...
        : def.orig.every(sym => !(sym instanceof AST_SymbolConst || sym instanceof AST_SymbolDefun || sym instanceof AST_SymbolLambda)));
  }
  ...
  function mark_escaped(tw, d, scope, node, value, level, depth) {
    ...
    if (
      (parent instanceof AST_Assign && parent.operator == "=" && node === parent.right) ||
    ...
    if (
      ...
      mark_escaped(tw, d, scope, parent, (value = read_property(value, parent.property)), level + 1, depth + 1);
      if (value) return;
    }
    level > 0 ||
      (parent instanceof AST_Sequence && node !== parent.tail_node()) ||
  ...
  const suppress = node => walk(node, node => {
    ...
    if (!d) return;
    node instanceof AST_SymbolRef && d.references.push(node);
    d.fixed = false;
  });
  ...
  def_reduce_vars(AST_Assign, function(tw, descend, compressor) {
    ...
    var def = sym.definition(),
      safe = safe_to_assign(tw, def, sym.scope, node.right);
    def.assignments++;
    if (!safe) return;
    var fixed = def.fixed;
    if (!fixed && node.operator != "=") return;
    var eq = node.operator == "=",
      value = eq ? node.right : node;
  ...
  def_reduce_vars(AST_For, function(tw, descend, compressor) {
    reset_block_variables(compressor, this);
    this.init && this.init.walk(tw);
    const saved_loop = tw.in_loop;
    tw.in_loop = this;
    push(tw);
    this.condition && this.condition.walk(tw);
    this.body.walk(tw);
    if (this.step) {
      if (has_break_or_continue(this)) {
        pop(tw);
  ...
  def_reduce_vars(AST_SymbolRef, function(tw, descend, compressor) {
    ...
    d.references.length == 1 && !d.fixed && d.orig[0] instanceof AST_SymbolDefun && tw.loop_ids.set(d.id, tw.in_loop);

    if (d.fixed === void 0 || !safe_to_read(tw, d)) d.fixed = false;
    else if (d.fixed) {
      (fixed_value = this.fixed_value()) instanceof AST_Lambda && recursive_ref(tw, d) ? d.recursive_refs++
        : fixed_value && !compressor.exposed(d) && ref_once(tw, compressor, d)
        ? (d.single_use =
      ...
      if (is_modified(compressor, tw, this, fixed_value, 0, is_immutable(fixed_value)))
        d.single_use ? (d.single_use = "m") : (d.fixed = false);
    }
  ...
  def_reduce_vars(AST_Try, function(tw, descend, compressor) {
    ...
    this.bfinally && this.bfinally.walk(tw);
    return true;
  });
  def_reduce_vars(AST_Unary, function(tw) {
    ...
    var def = exp.definition(),
      safe = safe_to_assign(tw, def, exp.scope, true);
    def.assignments++;
    if (!safe) return;
    var fixed = def.fixed;
    if (!fixed) return;
    ...
    def.fixed = function() {
      return make_node(AST_Binary, node, {
        operator: node.operator.slice(0, -1),
        left: make_node(AST_UnaryPrefix, node, { operator: "+", expression: fixed instanceof AST_Node ? fixed : fixed() }),
...
AST_Toplevel.DEFMETHOD("reset_opt_flags", function(compressor) {
  ...
  const preparation = new TreeWalker(function(node, descend) {
    ...
    if (reduce_vars) {
      compressor.top_retain && node instanceof AST_Defun && preparation.parent() === self && set_flag(node, TOP);
...
function is_ref_of(ref, type) {
  ...
  for (var orig = ref.definition().orig, i = orig.length; --i >= 0; ) if (orig[i] instanceof type) return true;
}
...
function find_variable(compressor, name) {
  var scope, i = 0;
  while ((scope = compressor.parent(i++)) && !(scope instanceof AST_Scope))
...
function make_node_from_constant(val, orig) {
  ...
  case "number":
    return isNaN(val) ? make_node(AST_NaN, orig)
      : isFinite(val)
      ? 1 / val < 0
        ? make_node(AST_UnaryPrefix, orig, { operator: "-", expression: make_node(AST_Number, orig, { value: -val }) })
        : make_node(AST_Number, orig, { value: val })
      : val < 0
      ? make_node(AST_UnaryPrefix, orig, { operator: "-", expression: make_node(AST_Infinity, orig) })
  ...
  default:
    ...
    if (val instanceof RegExp)
      return make_node(AST_RegExp, orig, { value: { source: regexp_source_fix(val.source), flags: val.flags } });
...
function maintain_this_binding(parent, orig, val) {
  return (parent instanceof AST_UnaryPrefix && parent.operator == "delete") ||
    ...
    ? make_sequence(orig, [make_node(AST_Number, orig, { value: 0 }), val])
    : val;
}

function merge_sequence(array, node) {
  node instanceof AST_Sequence ? array.push(...node.expressions) : array.push(node);
...
function is_empty(thing) {
  return thing === null || thing instanceof AST_EmptyStatement || (thing instanceof AST_BlockStatement && thing.body.length == 0);
}
...
function is_iife_call(node) {
  return node.TYPE == "Call" && (node.expression instanceof AST_Function || is_iife_call(node.expression));
}
...
function tighten_body(statements, compressor) {
  var in_loop, in_try,
    scope = compressor.find_parent(AST_Scope).get_defun_scope();
  find_loop_scope_try();
  var CHANGED, max_iter = 10;
  do {
    CHANGED = false;
    eliminate_spurious_blocks(statements);
    compressor.option("dead_code") && eliminate_dead_code(statements, compressor);

    compressor.option("if_return") && handle_if_return(statements, compressor);

    if (compressor.sequences_limit > 0) {
      sequencesize(statements, compressor);
      sequencesize_2(statements, compressor);
    }
    compressor.option("join_vars") && join_consecutive_vars(statements);
  ...
  function find_loop_scope_try() {
    ...
    do {
      if (node instanceof AST_Catch || node instanceof AST_Finally) level++;
      else if (node instanceof AST_IterationStatement) in_loop = true;
  ...
  function collapse(statements, compressor) {
    ...
    var scanner = new TreeTransformer(function(node) {
      ...
      if (!hit) {
        ...
        hit = true;
        if ((stop_after = find_stop(node, 0)) === node) abort = true;
        return node;
      }
      var sym,
        parent = scanner.parent();
      if (
        (node instanceof AST_Assign && node.operator != "=" && lhs.equivalent_to(node.left)) ||
        node instanceof AST_Await ||
        (node instanceof AST_Call && lhs instanceof AST_PropAccess && lhs.equivalent_to(node.expression)) ||
        node instanceof AST_Debugger ||
        node instanceof AST_Destructuring ||
        (node instanceof AST_Expansion &&
          node.expression instanceof AST_Symbol && node.expression.definition().references.length > 1) ||
        ...
        (!replace_all && node instanceof AST_SymbolRef &&
          !node.is_declared(compressor) && !pure_prop_access_globals.has(node)) ||
        (node instanceof AST_SymbolRef && parent instanceof AST_Call && has_annotation(parent, _NOINLINE))
      ) {
        abort = true;
        return node;
      }
      ...
      if (can_replace && !(node instanceof AST_SymbolDeclaration) && lhs.equivalent_to(node)) {
        ...
        if (candidate instanceof AST_VarDef) {
          ...
          if (def.references.length - def.replaced == 1 && !compressor.exposed(def)) {
            def.replaced++;
            return funarg && is_identifier_atom(value) ? value.transform(compressor) : maintain_this_binding(parent, node, value);
          }
      ...
      if (
        ...
        (node instanceof AST_Exit && (side_effects || lhs instanceof AST_PropAccess || may_modify(lhs))) ||
        (node instanceof AST_PropAccess && (side_effects || node.expression.may_throw_on_access(compressor))) ||
        (node instanceof AST_SymbolRef && (lvalues.get(node.name) || (side_effects && may_modify(node)))) ||
        (node instanceof AST_VarDef && node.value &&
          (lvalues.has(node.name.name) || (side_effects && may_modify(node.name)))) ||
        ((sym = is_lhs(node.left, node)) && (sym instanceof AST_PropAccess || lvalues.has(sym.name))) ||
    ...
    while (--stat_index >= 0) {
      stat_index == 0 && compressor.option("unused") && extract_args();
      var hit_stack = [];
      ...
      while (candidates.length > 0) {
        ...
        lhs instanceof AST_SymbolRef && lvalues.set(lhs.name, false);
        var side_effects = value_has_side_effects(candidate),
          ...
          hit = funarg,
          abort = false, replaced = 0, can_replace = !args || !hit;
        if (!can_replace) {
    ...
    function redefined_within_scope(def, scope) {
      ...
      for (let cur_scope = def.scope; cur_scope && cur_scope !== scope; ) {
    ...
    function extract_args() {
      ...
      if (
        ...
        for (var names = new Set(), i = len; --i >= 0; ) {
          ...
          if (def && def.orig.length > 1) continue;
          args.unshift(make_node(AST_VarDef, sym, { name: sym, value: arg }));
          if (names.has(sym.name)) continue;
          names.add(sym.name);
          if (sym instanceof AST_Expansion) {
            var elements = iife.args.slice(i);
            elements.every(arg => !has_overlapping_symbol(fn, arg, fn_strict)) &&
              candidates.unshift([
                make_node(AST_VarDef, sym, { name: sym.expression, value: make_node(AST_Array, iife, { elements: elements }) })
              ]);
          } else {
            if (!arg) arg = make_node(AST_Undefined, sym).transform(compressor);
            else if ((arg instanceof AST_Lambda && arg.pinned()) || has_overlapping_symbol(fn, arg, fn_strict)) arg = null;

            arg && candidates.unshift([make_node(AST_VarDef, sym, { name: sym, value: arg })]);
          }
        }
      }
    }
    ...
    function extract_candidates(expr) {
      ...
      if (expr instanceof AST_Assign) {
        expr.left.has_side_effects(compressor) || candidates.push(hit_stack.slice());
        ...
        extract_candidates(expr.condition);
        expr.body instanceof AST_Block || extract_candidates(expr.body);
      } else if (expr instanceof AST_Exit) expr.value && extract_candidates(expr.value);
      else if (expr instanceof AST_For) {
        expr.init && extract_candidates(expr.init);
        expr.condition && extract_candidates(expr.condition);
        expr.step && extract_candidates(expr.step);
        expr.body instanceof AST_Block || extract_candidates(expr.body);
        ...
        !expr.alternative || expr.alternative instanceof AST_Block || extract_candidates(expr.alternative);
      } else if (expr instanceof AST_Sequence) expr.expressions.forEach(extract_candidates);
      else if (expr instanceof AST_SimpleStatement) extract_candidates(expr.body);
      else if (expr instanceof AST_Switch) {
        extract_candidates(expr.expression);
        expr.body.forEach(extract_candidates);
      } else if (expr instanceof AST_Unary)
        (expr.operator != "++" && expr.operator != "--") || candidates.push(hit_stack.slice());
      else if (expr instanceof AST_VarDef && expr.value) {
    ...
    function find_stop(node, level, write_only) {
      ...
      return parent instanceof AST_Assign
        ? write_only && !(parent.left instanceof AST_PropAccess || lvalues.has(parent.left.name))
        ...
        ? write_only && (!lazy_op.has(parent.operator) || parent.left === node)
          ? find_stop(parent, level + 1, write_only)
          : node
        : parent instanceof AST_Call || parent instanceof AST_Case
        ? node
        : parent instanceof AST_Conditional
        ? write_only && parent.condition === node
          ? find_stop(parent, level + 1, write_only)
          : node
        : parent instanceof AST_Definitions
        ? find_stop(parent, level + 1, true)
        : parent instanceof AST_Exit
        ? write_only ? find_stop(parent, level + 1, write_only) : node
        ...
        ? find_stop(parent, level + 1, parent.tail_node() !== node)
        : parent instanceof AST_SimpleStatement
        ? find_stop(parent, level + 1, true)
        : parent instanceof AST_Switch || parent instanceof AST_VarDef
        ? node
        : null;
    }
    ...
    function mangleable_var(var_def) {
      ...
      if (!(value instanceof AST_SymbolRef) || value.name == "arguments") return;
    ...
    function get_lhs(expr) {
      ...
      if (referenced &&
        ((def.orig.length - def.eliminated > 1 && !(expr.name instanceof AST_SymbolFunarg)) ||
    ...
    function remove_candidate(expr) {
      if (expr.name instanceof AST_SymbolFunarg) {
        var iife = compressor.parent(), argnames = compressor.self().argnames,
          index = argnames.indexOf(expr.name);
    ...
    function is_lhs_local(lhs) {
      ...
      return (
        ...
        !(in_loop && (lvalues.has(lhs.name) || candidate instanceof AST_Unary ||
    ...
    function may_modify(sym) {
      ...
      var def = sym.definition();
      return !(def.orig.length == 1 && def.orig[0] instanceof AST_SymbolDefun) &&
        (def.scope.get_defun_scope() !== scope || !def.references.every(ref => {
  ...
  function handle_if_return(statements, compressor) {
    var self = compressor.self(),
      multiple_if_returns = has_multiple_if_returns(statements),
    ...
    for (var i = statements.length; --i >= 0; ) {
      ...
      if (stat instanceof AST_If) {
        ...
        if (can_merge_flow((ab = aborts(stat.body)))) {
          ab.label && remove(ab.label.thedef.references, ab);

          CHANGED = true;
          (stat = stat.clone()).condition = stat.condition.negate(compressor);
        ...
        if (can_merge_flow((ab = aborts(stat.alternative)))) {
          ...
          CHANGED = true;
          (stat = stat.clone()).body = make_node(AST_BlockStatement, stat.body, {
      ...
      if (stat instanceof AST_If && stat.body instanceof AST_Return) {
        ...
        if (value && !stat.alternative && next instanceof AST_Return && next.value) {
          CHANGED = true;
          (stat = stat.clone()).alternative = next;
          statements[i] = stat.transform(compressor);
        ...
        if (value && !stat.alternative && ((!next && in_lambda && multiple_if_returns) || next instanceof AST_Return)) {
          CHANGED = true;
          (stat = stat.clone()).alternative = next || make_node(AST_Return, stat, { value: null });
          statements[i] = stat.transform(compressor);
          next && statements.splice(j, 1);
          continue;
        }
        ...
        if (
          ...
          CHANGED = true;
          (stat = stat.clone()).alternative = make_node(AST_BlockStatement, next, {
    ...
    function has_multiple_if_returns(statements) {
      for (var n = 0, i = statements.length; --i >= 0; ) {
        var stat = statements[i];
        if (stat instanceof AST_If && stat.body instanceof AST_Return && ++n > 1) return true;
      }
      return false;
    }
    ...
    function as_statement_array_with_return(node, ab) {
      ...
      ab.value && body.push(make_node(AST_SimpleStatement, ab.value, { body: ab.value.expression }));
  ...
  function eliminate_dead_code(statements, compressor) {
    ...
    for (var i = 0, n = 0, len = statements.length; i < len; i++) {
      ...
      if (stat instanceof AST_LoopControl) {
        ...
        (stat instanceof AST_Break && !(lct instanceof AST_IterationStatement) && loop_body(lct) === self) ||
        (stat instanceof AST_Continue && loop_body(lct) === self)
          ? stat.label && remove(stat.label.thedef.references, stat)
          : (statements[n++] = stat);
    ...
    statements.length = n;
    CHANGED = n != len;
    has_quit && has_quit.forEach(function(stat) {
    ...
    for (var i = 0; i < statements.length; i++) {
      ...
      if (prev)
        if (stat instanceof AST_Exit)
          ...
          if (!(stat.init instanceof AST_Definitions) && !walk(prev.body, node =>
            node instanceof AST_Scope || (node instanceof AST_Binary && node.operator === "in" ? walk_abort : void 0)
          ))
          ...
          stat.init instanceof AST_Const || stat.init instanceof AST_Let || (stat.object = cons_seq(stat.object));
        else if (stat instanceof AST_If) stat.condition = cons_seq(stat.condition);
        else if (stat instanceof AST_Switch || stat instanceof AST_With) stat.expression = cons_seq(stat.expression);

      if (compressor.option("conditionals") && stat instanceof AST_If) {
        var decls = [],
          body = to_simple_statement(stat.body, decls),
          alt = to_simple_statement(stat.alternative, decls);
  ...
  function join_object_assignments(defn, body) {
    ...
    var exprs;
    if (body instanceof AST_Assign) exprs = [body];
    else if (body instanceof AST_Sequence) exprs = body.expressions.slice();
    ...
    do {
      ...
      var p = def.value.properties.filter((p) => p.key === prop)[0];
      !p
        ? def.value.properties.push(make_node(AST_ObjectKeyVal, node, { key: prop, value: node.right }))
        : (p.value = new AST_Sequence({ start: p.start, expressions: [p.value.clone(), node.right.clone()], end: p.end }));
  ...
  function join_consecutive_vars(statements) {
    for (var defs, i = 0, j = -1, len = statements.length; i < len; i++) {
      ...
      if (stat instanceof AST_Definitions)
        ...
        if ((exprs = join_object_assignments(prev, stat.init))) {
          CHANGED = true;
        ...
        statements[++j] = stat;
      } else stat instanceof AST_Switch || stat instanceof AST_With
    ...
    function extract_object_assignments(value) {
      ...
      if (exprs) {
        CHANGED = true;
        return exprs.length ? make_sequence(value, exprs)
          : value instanceof AST_Sequence ? value.tail_node().left : value.left;
      }
      return value;
    }
  }
  ...
  walk(stat, node => {
    ...
    if (node instanceof AST_Defun && (node === stat || !compressor.has_directive("use strict"))) {
      target.push(node === stat ? node : make_node(AST_Var, node, {
        definitions: [make_node(AST_VarDef, node, { name: make_node(AST_SymbolVar, node.name, node.name), value: null })]
      }));
      return true;
    }
...
function get_value(key) {
  return key instanceof AST_Constant
    ? key.getValue()
    : key instanceof AST_UnaryPrefix && key.operator == "void" && key.expression instanceof AST_Constant
    ? void 0
    : key;
}
...
function is_undefined(node, compressor) {
  return (
    ...
    (node instanceof AST_UnaryPrefix && node.operator == "void" && !node.expression.has_side_effects(compressor))
  );
}
...
!(function(def_may_throw_on_access) {
  ...
  def_may_throw_on_access(AST_Dot, function(compressor) {
    return !(!is_strict(compressor) || (this.expression instanceof AST_Function && this.property == "prototype"));
  });
  ...
  def_may_throw_on_access(AST_SymbolRef, function(compressor) {
    ...
    if (!is_strict(compressor) || (is_undeclared_ref(this) && this.is_declared(compressor)) || this.is_immutable())
      return false;
...
!(function(def_is_boolean) {
  const unary_bool = makePredicate("! delete"),
    binary_bool = makePredicate("in instanceof == != === !== < <= >= >");
  ...
  def_is_boolean(AST_Binary, function() {
    return binary_bool.has(this.operator) ||
      (lazy_op.has(this.operator) && this.left.is_boolean() && this.right.is_boolean());
  });
...
!(function(def_is_number) {
  ...
  def_is_number(AST_Binary, function(compressor) {
    return binary.has(this.operator) ||
      (this.operator == "+" && this.left.is_number(compressor) && this.right.is_number(compressor));
  });
...
var lazy_op = makePredicate("&& || ??"),
  unary_side_effects = makePredicate("delete ++ --");

function is_lhs(node, parent) {
  return parent instanceof AST_Unary && unary_side_effects.has(parent.operator) ? parent.expression
    : parent instanceof AST_Assign && parent.left === node ? node : void 0;
}

!(function(def_find_defs) {
  function to_node(value, orig) {
    if (value instanceof AST_Node) return make_node(value.CTOR, orig, value);
    if (Array.isArray(value)) return make_node(AST_Array, orig, { elements: value.map((value) => to_node(value, orig)) });
    if (value && typeof value == "object") {
      var props = [];
      for (var key in value)
        HOP(value, key) && props.push(make_node(AST_ObjectKeyVal, orig, { key: key, value: to_node(value[key], orig) }));
  ...
  AST_Toplevel.DEFMETHOD("resolve_defines", function(compressor) {
    ...
    return this.transform(new TreeTransformer(function(node) {
      ...
      if (!def) return;
      var parent, level = 0, child = node;
      while ((parent = this.parent(level++)) && parent instanceof AST_PropAccess && parent.expression === child)
        child = parent;
  ...
  def_find_defs(AST_SymbolDeclaration, function(compressor) {
    this.global() &&
      HOP(compressor.option("global_defs"), this.name) && warn(compressor, this);
  });
...
!(function(def_eval) {
  ...
  AST_Node.DEFMETHOD("is_constant", function() {
    return this instanceof AST_Constant
      ...
      : this instanceof AST_UnaryPrefix && this.expression instanceof AST_Constant && unaryPrefix.has(this.operator);
  });
    ...
    if (compressor.option("unsafe")) {
      ...
      for (var i = 0, len = this.elements.length; i < len; i++) {
        var element = this.elements[i],
          value = element._eval(compressor, depth);
  ...
  def_eval(AST_UnaryPrefix, function(compressor, depth) {
    ...
    if (
      ...
      (e instanceof AST_Lambda || (e instanceof AST_SymbolRef && e.fixed_value() instanceof AST_Lambda))
    )
      return "function"; // typeof function() {}

    non_converting_unary.has(this.operator) || depth++;
  ...
  def_eval(AST_Binary, function(compressor, depth) {
    ...
    var result,
      right = this.right._eval(compressor, depth);
  ...
  def_eval(AST_Conditional, function(compressor, depth) {
    ...
    var node = condition ? this.consequent : this.alternative,
      value = node._eval(compressor, depth);
  ...
  def_eval(AST_SymbolRef, function(compressor, depth) {
    ...
    if (HOP(fixed, "_eval")) value = fixed._eval();
    else {
  ...
  def_eval(AST_PropAccess, function(compressor, depth) {
    ...
    if (is_undeclared_ref(exp)) {
      var aa;
      var first_arg = exp.name === "hasOwnProperty" && key === "call" &&
        (aa = compressor.parent() && compressor.parent().args) && aa[0] && aa[0].evaluate(compressor);

      first_arg instanceof AST_Dot && (first_arg = first_arg.expression);

      if (first_arg == null || (first_arg.thedef && first_arg.thedef.undeclared)) return this.clone();
  ...
  def_eval(AST_Call, function(compressor, depth) {
    ...
    if (!(compressor.option("unsafe") && exp instanceof AST_PropAccess)) return this;
    var val,
      key = exp.property;
    ...
    for (var i = 0, len = this.args.length; i < len; i++) {
      var arg = this.args[i],
        value = arg._eval(compressor, depth);
...
AST_Call.DEFMETHOD("is_expr_pure", function(compressor) {
  if (compressor.option("unsafe")) {
    var expr = this.expression,
      first_arg = this.args && this.args[0] && this.args[0].evaluate(compressor);
    if (
...
AST_Dot.DEFMETHOD("is_call_pure", function(compressor) {
  ...
  expr instanceof AST_Array
    ...
    : expr.is_number(compressor)
    ? (map = native_fns.get("Number"))
    : expr instanceof AST_RegExp
    ? (map = native_fns.get("RegExp"))
    : expr.is_string(compressor)
    ? (map = native_fns.get("String"))
    : this.may_throw_on_access(compressor) ||
...
!(function(def_has_side_effects) {
  ...
  def_has_side_effects(AST_Call, function(compressor) {
    return !(this.is_expr_pure(compressor) ||
        (this.expression.is_call_pure(compressor) && !this.expression.has_side_effects(compressor))) ||
...
!(function(def_may_throw) {
  ...
  def_may_throw(AST_Assign, function(compressor) {
    return !!this.right.may_throw(compressor) ||
      (!(!compressor.has_directive("use strict") && this.operator == "=" && this.left instanceof AST_SymbolRef) &&
  ...
  def_may_throw(AST_Call, function(compressor) {
    return !!any(this.args, compressor) ||
      (!this.is_expr_pure(compressor) &&
        (!(!this.expression.may_throw(compressor) && this.expression instanceof AST_Lambda) ||
  ...
  def_may_throw(AST_Sub, function(compressor) {
    return this.expression.may_throw_on_access(compressor) ||
      this.expression.may_throw(compressor) || this.property.may_throw(compressor);
  });
  ...
  def_may_throw(AST_Try, function(compressor) {
    return this.bcatch ? this.bcatch.may_throw(compressor)
      : any(this.body, compressor) || (this.bfinally && this.bfinally.may_throw(compressor));
  });
  def_may_throw(AST_Unary, function(compressor) {
    return !(this.operator == "typeof" && this.expression instanceof AST_SymbolRef) && this.expression.may_throw(compressor);
  });
  ...
  def_is_constant_expression(AST_Class, function(scope) {
    ...
    for (const prop of this.properties)
      if ((prop.computed_key() && !prop.key.is_constant_expression(scope)) ||
          (prop.static && prop.value && !prop.value.is_constant_expression(scope))) return false;
...
def_optimize(AST_Directive, function(self, compressor) {
  return compressor.option("directives") && (!directives.has(self.value) || compressor.has_directive(self.value) !== self)
...
def_optimize(AST_LabeledStatement, function(self, compressor) {
  return self.body instanceof AST_Break && compressor.loopcontrol_target(self.body) === self.body
    ? make_node(AST_EmptyStatement, self)
    : self.label.references.length == 0 ? self.body : self;
});
...
def_optimize(AST_BlockStatement, function(self, compressor) {
  ...
  case 1:
    if ((!compressor.has_directive("use strict") &&
        compressor.parent() instanceof AST_If && can_be_extracted_from_if_block(self.body[0])) ||
...
AST_Scope.DEFMETHOD("drop_unused", function(compressor) {
  ...
  var drop_funcs = !(self instanceof AST_Toplevel) || compressor.toplevel.funcs !== false,
    drop_vars = !(self instanceof AST_Toplevel) || compressor.toplevel.vars !== false;
  const assign_as_unused = r_keep_assign.test(compressor.option("unused")) ? return_false : function(node) {
    return node instanceof AST_Assign && (has_flag(node, WRITE_ONLY) || node.operator == "=") ? node.left

      : node instanceof AST_Unary && has_flag(node, WRITE_ONLY) ? node.expression : void 0;
  };
  ...
  var var_defs_by_id = new Map(),
    ...
    scope = this;
  var tw = new TreeWalker(function(node, descend) {
    node instanceof AST_Lambda && node.uses_arguments && !tw.has_directive("use strict") &&
    ...
    if (node instanceof AST_Defun || node instanceof AST_DefClass) {
      ...
      (tw.parent() instanceof AST_Export || (!drop_funcs && scope === self)) && node_def.global && !in_use_ids.has(node_def.id) &&
        in_use_ids.set(node_def.id, node_def);

      if (node instanceof AST_DefClass) {
        node.extends && (node.extends.has_side_effects(compressor) || node.extends.may_throw(compressor)) &&
          node.extends.walk(tw);

        for (const prop of node.properties) if (prop.has_side_effects(compressor) || prop.may_throw(compressor)) prop.walk(tw);
      }
      map_add(initializations, node_def.id, node);
      return true;
    }
    node instanceof AST_SymbolFunarg && scope === self && map_add(var_defs_by_id, node.definition().id, node);

    if (node instanceof AST_Definitions && scope === self) {
      const in_export = tw.parent() instanceof AST_Export;
      node.definitions.forEach(function(def) {
        def.name instanceof AST_SymbolVar && map_add(var_defs_by_id, def.name.definition().id, def);
        ...
        if (def.value) {
          if (def.name instanceof AST_Destructuring) def.walk(tw);
            ...
            node_def.chained || def.name.fixed_value() !== def.value || fixed_ids.set(node_def.id, def);
          }
  ...
  var tt = new TreeTransformer(function(node, descend, in_list) {
    ...
    if (drop_vars) {
      ...
      if (sym instanceof AST_SymbolRef) {
        var def = sym.definition(),
          in_use = in_use_ids.has(def.id);
        if (node instanceof AST_Assign) {
          if (!in_use || (fixed_ids.has(def.id) && fixed_ids.get(def.id) !== node))
            return maintain_this_binding(parent, node, node.right.transform(tt));
        } else if (!in_use) return in_list ? MAP.skip : make_node(AST_Number, node, { value: 0 });
      }
    }
    if (scope !== self) return;
    if (
      node.name &&
      ((node instanceof AST_ClassExpression && !keep_name(compressor.option("keep_classnames"), (def = node.name.definition()).name)) ||
        (node instanceof AST_Function && !keep_name(compressor.option("keep_fnames"), (def = node.name.definition()).name))) &&
      (!in_use_ids.has(def.id) || def.orig.length > 1)
    )
      node.name = null;

    if (node instanceof AST_Lambda && !(node instanceof AST_Accessor))
      for (var trim = !compressor.option("keep_fargs"), a = node.argnames, i = a.length; --i >= 0; ) {
        var sym = a[i];
        if (sym instanceof AST_Expansion) sym = sym.expression;
        if (sym instanceof AST_DefaultAssign) sym = sym.left;

        if (sym instanceof AST_Destructuring || in_use_ids.has(sym.definition().id)) trim = false;
        else {
          set_flag(sym, UNUSED);
          if (trim) {
            a.pop();
    ...
    if ((node instanceof AST_Defun || node instanceof AST_DefClass) && node !== self) {
      ...
      if (!((def.global && !drop_funcs) || in_use_ids.has(def.id))) {
        ...
        if (node instanceof AST_DefClass) {
          ...
          if (side_effects) return make_node(AST_SimpleStatement, node, { body: side_effects });
        }
    ...
    if (node instanceof AST_Definitions && !(parent instanceof AST_ForIn && parent.init === node)) {
      ...
      node.definitions.forEach(function(def) {
        ...
        var value,
          is_destructure = def.name instanceof AST_Destructuring,
          sym = is_destructure ? new SymbolDef(null, { name: "<destructure>" }) : def.name.definition();
        if (drop_block && sym.global) return tail.push(def);
        if (
          (!drop_vars && !drop_block) ||
          (is_destructure && (def.name.names.length || def.name.is_array || compressor.option("pure_getters") != 1)) ||
          ...
          if (def.name instanceof AST_SymbolVar) {
            ...
            if (var_defs.length > 1 && (!def.value || sym.orig.indexOf(def.name) > sym.eliminated)) {
              ...
              if (def.value) {
                ...
                fixed_ids.get(sym.id) !== def || fixed_ids.set(sym.id, assign);
          ...
          if (def.value) {
            if (side_effects.length > 0) {
              ...
              } else body.push(make_node(AST_SimpleStatement, node, { body: make_sequence(node, side_effects) }));
          ...
          if ((value = def.value && def.value.drop_side_effect_free(compressor))) {
            is_destructure ||
      ...
      side_effects.length > 0 && body.push(make_node(AST_SimpleStatement, node, { body: make_sequence(node, side_effects) }));
    ...
    if (node instanceof AST_For) {
      ...
      if (node.init instanceof AST_SimpleStatement) node.init = node.init.body;
      else if (is_empty(node.init)) node.init = null;
    ...
    if (node instanceof AST_BlockStatement) {
      descend(node, this);
      return in_list && node.body.every(can_be_evicted_from_block) ? MAP.splice(node.body) : node;
    }
    if (node instanceof AST_Scope) {
      const save_scope = scope;
      scope = node;
...
AST_Scope.DEFMETHOD("hoist_declarations", function(compressor) {
  ...
  if (hoist_funs || hoist_vars) {
    var dirs = [],
      hoisted = [],
      vars = new Map(), vars_found = 0, var_decl = 0;
    walk(self, node => {
    ...
    var tt = new TreeTransformer(function(node) {
      ...
      if (hoist_vars && node instanceof AST_Var) {
        ...
        return p instanceof AST_For && p.init === node ? seq
          : !seq ? make_node(AST_EmptyStatement, node)
          : make_node(AST_SimpleStatement, node, { body: seq });
      }
      if (node instanceof AST_Scope) return node;
    });
    ...
    if (vars_found > 0) {
      ...
      const is_lambda = self instanceof AST_Lambda,
        args_as_names = is_lambda ? self.args_as_names() : null;
      vars.forEach((def, name) => {
        if (is_lambda && args_as_names.some(x => x.name === def.name.name)) vars.delete(name);
        else {
          (def = def.clone()).value = null;
          defs.push(def);
          vars.set(name, def);
        }
      });
      if (defs.length > 0) {
        for (var i = 0; i < self.body.length; ) {
          if (self.body[i] instanceof AST_SimpleStatement) {
            var def, sym, assign, expr = self.body[i].body;
            if (expr instanceof AST_Assign && expr.operator == "=" &&
                (sym = expr.left) instanceof AST_Symbol && vars.has(sym.name)) {
          ...
          else if (self.body[i] instanceof AST_BlockStatement) {
...
AST_Scope.DEFMETHOD("hoist_properties", function(compressor) {
  ...
  var hoister = new TreeTransformer(function(node, descend) {
    ...
    if (node instanceof AST_VarDef) {
      ...
      if (
        ...
        value.properties.forEach(function(prop) {
          assignments.push(make_node(AST_VarDef, node, { name: make_sym(sym, prop.key, defs), value: prop.value }));
        });
      ...
      if (defs) {
        const def = defs.get(String(get_value(node.property))),
          sym = make_node(AST_SymbolRef, node, { name: def.name, scope: node.expression.scope, thedef: def });
        sym.reference({});
        return sym;
      }
    }
    ...
    function make_sym(sym, key, defs) {
      const new_var = make_node(sym.CTOR, sym, { name: self.make_var_name(sym.name + "_" + key), scope: self }),
...
!(function(def_drop_side_effect_free) {
  ...
  def_drop_side_effect_free(AST_Call, function(compressor, first_in_statement) {
    if (!this.is_expr_pure(compressor)) {
      if (this.expression.is_call_pure(compressor)) {
        ...
        return (exprs = trim(exprs, compressor, first_in_statement)) && make_sequence(this, exprs);
      }
  ...
  def_drop_side_effect_free(AST_Class, function(compressor) {
    const with_effects = [],
      trimmed_extends = this.extends && this.extends.drop_side_effect_free(compressor);
    trimmed_extends && with_effects.push(trimmed_extends);
    for (const prop of this.properties) {
      const trimmed_prop = prop.drop_side_effect_free(compressor);
      trimmed_prop && with_effects.push(trimmed_prop);
    }
    return !with_effects.length ? null : make_sequence(this, with_effects);
  });
  def_drop_side_effect_free(AST_Binary, function(compressor, first_in_statement) {
    ...
    return !left ? this.right.drop_side_effect_free(compressor, first_in_statement) : make_sequence(this, [left, right]);
  });
  def_drop_side_effect_free(AST_Assign, function(compressor) {
    var left = this.left;
    if (left.has_side_effects(compressor) ||
        (compressor.has_directive("use strict") && left instanceof AST_PropAccess && left.expression.is_constant()))
      return this;
  ...
  def_drop_side_effect_free(AST_Conditional, function(compressor) {
    ...
    if (!consequent) return alternative
      ? make_node(AST_Binary, this, { operator: "||", left: this.condition, right: alternative })
  ...
  def_drop_side_effect_free(AST_Unary, function(compressor, first_in_statement) {
    if (unary_side_effects.has(this.operator)) {
      !this.expression.has_side_effects(compressor) ? set_flag(this, WRITE_ONLY) : clear_flag(this, WRITE_ONLY);
      return this;
    }
    if (this.operator == "typeof" && this.expression instanceof AST_SymbolRef) return null;
    var expression = this.expression.drop_side_effect_free(compressor, first_in_statement);
    return first_in_statement && expression && is_iife_call(expression)
      ? expression === this.expression && this.operator == "!" ? this
        : expression.negate(compressor, first_in_statement)
      : expression;
  });
  ...
  def_drop_side_effect_free(AST_ObjectProperty, function(compressor, first_in_statement) {
    const key = this instanceof AST_ObjectKeyVal && this.key instanceof AST_Node &&
        this.key.drop_side_effect_free(compressor, first_in_statement),
      value = this.value.drop_side_effect_free(compressor, first_in_statement);
  ...
  def_drop_side_effect_free(AST_ClassProperty, function(compressor) {
    const key = this.computed_key() && this.key.drop_side_effect_free(compressor),
      value = this.static && this.value && this.value.drop_side_effect_free(compressor);
  ...
  def_drop_side_effect_free(AST_Dot, function(compressor, first_in_statement) {
    return this.expression.may_throw_on_access(compressor) ? this
      : this.expression.drop_side_effect_free(compressor, first_in_statement);
  });
  def_drop_side_effect_free(AST_Sub, function(compressor, first_in_statement) {
    ...
    return !property ? expression : make_sequence(this, [expression, property]);
  });
  def_drop_side_effect_free(AST_Sequence, function(compressor) {
    var last = this.tail_node(),
      expr = last.drop_side_effect_free(compressor);
    if (expr === last) return this;
    var expressions = this.expressions.slice(0, -1);
    expr && expressions.push(expr);
    return !expressions.length ? make_node(AST_Number, this, { value: 0 }) : make_sequence(this, expressions);
  });
...
def_optimize(AST_SimpleStatement, function(self, compressor) {
  if (compressor.option("side_effects")) {
    var body = self.body,
      node = body.drop_side_effect_free(compressor, true);
    if (!node) {
...
function has_break_or_continue(loop, parent) {
  var found = false;
  var tw = new TreeWalker(function(node) {
    return !!found || node instanceof AST_Scope ||
      (node instanceof AST_LoopControl && tw.loopcontrol_target(node) === loop ? (found = true) : void 0);
  });
  parent instanceof AST_LabeledStatement && tw.push(parent);
  tw.push(loop);
  loop.body.walk(tw);
  return found;
}
  ...
  if (!(cond instanceof AST_Node)) {
    if (cond) return make_node(AST_For, self, {
      body: make_node(AST_BlockStatement, self.body, {
        body: [self.body, make_node(AST_SimpleStatement, self.condition, { body: self.condition })]
      })
    }).optimize(compressor);
...
function if_break_in_loop(self, compressor) {
  ...
  if (compressor.option("dead_code") && is_break(first)) {
    var body = [];
    self.init instanceof AST_Statement ? body.push(self.init)
      : self.init && body.push(make_node(AST_SimpleStatement, self.init, { body: self.init }));

    self.condition && body.push(make_node(AST_SimpleStatement, self.condition, { body: self.condition }));
  ...
  function drop_it(rest) {
    ...
    } else self.body = make_node(AST_BlockStatement, self.body, { body: rest }).transform(compressor);
...
def_optimize(AST_If, function(self, compressor) {
  ...
  if (compressor.option("dead_code")) {
    ...
    if (!cond) {
      ...
      extract_declarations_from_unreachable_code(compressor, self.body, body);
      body.push(make_node(AST_SimpleStatement, self.condition, { body: self.condition }));
      self.alternative && body.push(self.alternative);
      return make_node(AST_BlockStatement, self, { body: body }).optimize(compressor);
    }
    if (!(cond instanceof AST_Node)) {
      ...
      (body = []).push(make_node(AST_SimpleStatement, self.condition, { body: self.condition }));
      body.push(self.body);
      self.alternative && extract_declarations_from_unreachable_code(compressor, self.alternative, body);

      return make_node(AST_BlockStatement, self, { body: body }).optimize(compressor);
    }
  }
  var negated = self.condition.negate(compressor),
    self_condition_length = self.condition.size(),
    negated_length = negated.size(),
  ...
  if (is_empty(self.alternative) && self.body instanceof AST_SimpleStatement) {
    ...
    return negated_is_best
      ? make_node(AST_SimpleStatement, self, {
          body: make_node(AST_Binary, self, { operator: "||", left: negated, right: self.body.body })
        }).optimize(compressor)
      : compressor.option("conditionals") !== true && self.body.body instanceof AST_Assign
      ? self
      : make_node(AST_SimpleStatement, self, {
...
def_optimize(AST_Switch, function(self, compressor) {
  ...
  var branch,
    value = self.expression.evaluate(compressor);
  ...
  for (var i = 0, len = self.body.length; i < len && !exact_match; i++) {
    if ((branch = self.body[i]) instanceof AST_Default)
  ...
  while ((branch = body[body.length - 1])) {
    ...
    stat instanceof AST_Break && compressor.loopcontrol_target(stat) === self && branch.body.pop();
  ...
  if (body.length == 0)
    return make_node(AST_BlockStatement, self, {
      body: decl.concat(make_node(AST_SimpleStatement, self.expression, { body: self.expression }))
    }).optimize(compressor);
  ...
  if (body.length == 1 && (body[0] === exact_match || body[0] === default_branch)) {
    ...
    if (!has_break) {
      ...
      (exp = body[0].expression) && statements.unshift(make_node(AST_SimpleStatement, exp, { body: exp }));
  ...
  function eliminate_branch(branch, prev) {
    prev && !aborts(prev) ? (prev.body = prev.body.concat(branch.body))
...
def_optimize(AST_Try, function(self, compressor) {
  ...
  if (compressor.option("dead_code") && self.body.every(is_empty)) {
    var body = [];
    self.bcatch && extract_declarations_from_unreachable_code(compressor, self.bcatch, body);

    self.bfinally && body.push(...self.bfinally.body);
    return make_node(AST_BlockStatement, self, { body: body }).optimize(compressor);
  }
  return self;
});
...
AST_Definitions.DEFMETHOD("remove_initializers", function() {
  ...
  this.definitions.forEach(function(def) {
    if (def.name instanceof AST_SymbolDeclaration) {
      ...
      walk(def.name, node => {
        node instanceof AST_SymbolDeclaration && decls.push(make_node(AST_VarDef, def, { name: node, value: null }));
      });
  });
...
AST_Definitions.DEFMETHOD("to_assignments", function(compressor) {
  ...
  var assignments = this.definitions.reduce(function(a, def) {
    if (def.value && !(def.name instanceof AST_Destructuring)) {
      ...
      var varDef = make_node(AST_VarDef, def, { name: def.name, value: def.value }),
        var_ = make_node(AST_Var, def, { definitions: [varDef] });
      a.push(var_);
    }
    (def = def.name.definition()).eliminated++;
    def.replaced--;
    return a;
  }, []);
  return assignments.length == 0 ? null : make_sequence(this, assignments);
});

def_optimize(AST_Definitions, function(self) {
  return self.definitions.length == 0 ? make_node(AST_EmptyStatement, self) : self;
});
...
def_optimize(AST_Call, function(/** (AST_Call|AST_Node) */ self, compressor) {
  ...
  var node,
    is_func = fn instanceof AST_Lambda;
    ...
    for (var i = 0, len = self.args.length; i < len; i++) {
      if (fn.argnames[i] instanceof AST_Expansion) {
        if (has_flag(fn.argnames[i].expression, UNUSED))
          while (i < len) (node = self.args[i++].drop_side_effect_free(compressor)) && (self.args[pos++] = node);
      ...
      if (trim || has_flag(fn.argnames[i], UNUSED)) {
        if ((node = self.args[i].drop_side_effect_free(compressor))) self.args[pos++] = node;
        else if (!trim) {
  ...
  if (compressor.option("unsafe"))
    ...
    case "Array":
      if (self.args.length != 1) return make_node(AST_Array, self, { elements: self.args }).optimize(compressor);
    ...
    case "Number":
      ...
      if (self.args.length == 1 && compressor.option("unsafe_math"))
        return make_node(AST_UnaryPrefix, self, { expression: self.args[0], operator: "+" }).optimize(compressor);
    ...
    case "Boolean":
      ...
      if (self.args.length == 1) return make_node(AST_UnaryPrefix, self, {
        expression: make_node(AST_UnaryPrefix, self, { expression: self.args[0], operator: "!" }),
        operator: "!"
      }).optimize(compressor);
      break;
    case "RegExp":
      ...
      if (
        ...
        const rx = make_node(AST_RegExp, self, { value: { source, flags } });
    ...
    case "join":
      if (exp.expression instanceof AST_Array) EXIT: {
        var separator;
        if (self.args.length > 0 && (separator = self.args[0].evaluate(compressor)) === self.args[0]) break EXIT;
        ...
        consts.length > 0 && elements.push(make_node(AST_String, self, { value: consts.join(separator) }));
        if (elements.length == 0) return make_node(AST_String, self, { value: "" });
        if (elements.length == 1)
          return elements[0].is_string(compressor) ? elements[0] : make_node(AST_Binary, elements[0], {
            operator: "+",
            left: make_node(AST_String, self, { value: "" }),
            right: elements[0]
          });
        ...
        if (separator == "") {
          var first = elements[0].is_string(compressor) || elements[1].is_string(compressor)
            ? elements.shift()
        ...
        (node = self.clone()).expression = node.expression.clone();
    ...
    case "charAt":
      if (exp.expression.is_string(compressor)) {
        var arg = self.args[0],
          index = arg ? arg.evaluate(compressor) : 0;
        if (index !== arg)
    ...
    case "apply":
      if (self.args.length == 2 && self.args[1] instanceof AST_Array) {
        (args = self.args[1].elements.slice()).unshift(self.args[0]);
        return make_node(AST_Call, self, {
          expression: make_node(AST_Dot, exp, { expression: exp.expression, property: "call" }),
          args: args
        }).optimize(compressor);
      }
      break;
    case "call":
      ...
      if (func instanceof AST_Lambda && !func.contains_this())
        return (self.args.length
          ? make_sequence(this, [self.args[0], make_node(AST_Call, self, { expression: exp.expression, args: self.args.slice(1) })])
          : make_node(AST_Call, self, { expression: exp.expression, args: [] })
        ).optimize(compressor);
  ...
  if (compressor.option("unsafe_Function") && is_undeclared_ref(exp) && exp.name == "Function") {
    if (self.args.length == 0) return make_node(AST_Function, self, { argnames: [], body: [] }).optimize(compressor);
    if (self.args.every(x => x instanceof AST_String))
      try {
        var ast = parse(
          "n(function(" + self.args.slice(0, -1).map((arg) => arg.value).join(",") + "){" + self.args[self.args.length - 1].value + "})"
        );
        ...
        self.args = [
          make_node(AST_String, self, { value: fun.argnames.map((arg) => arg.print_to_string()).join(",") }),
  ...
  var stat = is_func && fn.body[0],
    is_regular_func = is_func && !fn.is_generator && !fn.async,
    can_inline = is_regular_func && compressor.option("inline") && !self.is_expr_pure(compressor);
  if (can_inline && stat instanceof AST_Return) {
    let returned = stat.value;
    if (!returned || returned.is_constant_expression()) {
      returned = returned ? returned.clone(true) : make_node(AST_Undefined, self);
    ...
    if (
      ...
      let parent;
      return self.args[0] instanceof AST_PropAccess && (parent = compressor.parent()) instanceof AST_Call && parent.expression === self
        ? make_sequence(self, [make_node(AST_Number, self, { value: 0 }), self.args[0].optimize(compressor)])
  ...
  if (can_inline) {
    ...
    if (
      ...
      !(function() {
        for (let p, i = 0; (p = compressor.parent(i++)); ) {
  ...
  function return_value(stat) {
    return !stat ? make_node(AST_Undefined, self)
      : stat instanceof AST_Return ? !stat.value ? make_node(AST_Undefined, self) : stat.value.clone(true)

      : stat instanceof AST_SimpleStatement
      ? make_node(AST_UnaryPrefix, stat, { operator: "void", expression: stat.body.clone(true) })
      : void 0;
  }
  ...
  function can_flatten_body(stat) {
    var body = fn.body,
      len = body.length;
    if (compressor.option("inline") < 3) return len == 1 && return_value(stat);

    stat = null;
    for (var i = 0; i < len; i++) {
      var line = body[i];
      if (line instanceof AST_Var) {
        if (stat && !line.definitions.every(var_def => !var_def.value)) return false;
      } else {
  ...
  function can_inject_args(block_scoped, safe_to_inject) {
    for (var i = 0, len = fn.argnames.length; i < len; i++) {
      ...
      if (!safe_to_inject || block_scoped.has(arg.name) || identifier_atom.has(arg.name) || scope.var_names().has(arg.name))
        return false;
  ...
  function can_inject_args_values() {
    ...
    for (let i = 0, len = fn.argnames.length; i < len; i++) {
      var arg = fn.argnames[i];
      if (!(arg instanceof AST_DefaultAssign && has_flag(arg.left, UNUSED)) &&
          !(arg instanceof AST_Expansion && has_flag(arg.expression, UNUSED)) &&
          !has_flag(arg, UNUSED) &&
          arg_vals_outer_refs.has(arg.name))
        return false;
    }
    for (let i = 0, len = fn.body.length; i < len; i++) {
      ...
      if (stat instanceof AST_Var)
        for (var j = stat.definitions.length; --j >= 0; ) {
          ...
          if (name instanceof AST_Destructuring || arg_vals_outer_refs.has(name.name)) return false;
        }
    }
    return true;
  }
  ...
  function can_inject_symbols() {
    ...
    do {
      (scope = compressor.parent(++level)).is_block_scope() && scope.block_scope &&
        scope.block_scope.variables.forEach(function(variable) {
          block_scoped.add(variable.name);
        });

      if (scope instanceof AST_Catch) scope.argname && block_scoped.add(scope.argname.name);
      else if (scope instanceof AST_IterationStatement) in_loop = [];
      else if (scope instanceof AST_SymbolRef && scope.fixed_value() instanceof AST_Scope) return false;
    } while (!(scope instanceof AST_Scope));

    var safe_to_inject = !(scope instanceof AST_Toplevel) || compressor.toplevel.vars,
      inline = compressor.option("inline");
    return !(
  ...
  function append_var(decls, expressions, name, value) {
    ...
    def.references.push(sym);
    value && expressions.push(make_node(AST_Assign, self, { operator: "=", left: sym, right: value.clone() }));
  }
  ...
  function flatten_args(decls, expressions) {
    ...
    for (i = len; --i >= 0; ) {
      var name = fn.argnames[i],
        value = self.args[i];
      if (has_flag(name, UNUSED) || !name.name || scope.var_names().has(name.name)) value && expressions.push(value);
      else {
      ...
      for (var j = 0, defs = stat.definitions.length; j < defs; j++) {
        ...
        if (in_loop && fn.argnames.every(argname => argname.name != name.name)) {
          var def = fn.variables.get(name.name),
            sym = make_node(AST_SymbolRef, name, name);
          def.references.push(sym);
...
def_optimize(AST_Sequence, function(self, compressor) {
  ...
  if (end == 0) {
    (self = maintain_this_binding(compressor.parent(), compressor.self(), expressions[0])) instanceof AST_Sequence ||
  ...
  function filter_for_side_effects() {
    var first = first_in_statement(compressor),
      last = self.expressions.length - 1;
...
def_optimize(AST_UnaryPrefix, function(self, compressor) {
  ...
  if (compressor.option("side_effects") && self.operator == "void") {
    if ((e = e.drop_side_effect_free(compressor))) {
      self.expression = e;
      return self;
    }
  ...
  if (compressor.in_boolean_context())
    ...
    case "typeof":
      ...
      return (
        e instanceof AST_SymbolRef ? make_node(AST_True, self) : make_sequence(self, [e, make_node(AST_True, self)])
      ).optimize(compressor);
    }

  if (self.operator == "-" && e instanceof AST_Infinity) e = e.transform(compressor);

  if (e instanceof AST_Binary && (self.operator == "+" || self.operator == "-") &&
  ...
  if (self.operator != "-" || !(e instanceof AST_Number || e instanceof AST_Infinity || e instanceof AST_BigInt)) {
    var ev = self.evaluate(compressor);
    if (ev !== self) return best_of(compressor, make_node_from_constant(ev, self).optimize(compressor), self);
  }
  return self;
});
...
AST_Binary.DEFMETHOD("lift_sequences", function(compressor) {
  ...
  if (this.left instanceof AST_Sequence) {
    var e, x = this.left.expressions.slice();
    (e = this.clone()).left = x.pop();
    x.push(e);
    return make_sequence(this, x).optimize(compressor);
  }
  if (this.right instanceof AST_Sequence && !this.left.has_side_effects(compressor)) {
    var assign = this.operator == "=" && this.left instanceof AST_SymbolRef,
      last = (x = this.right.expressions).length - 1;
    for (var i = 0; i < last && (assign || !x[i].has_side_effects(compressor)); i++);

    if (i == last) {
      x = x.slice();
      (e = this.clone()).right = x.pop();
      x.push(e);
      return make_sequence(this, x).optimize(compressor);
    }
    if (i > 0) {
      (e = this.clone()).right = make_sequence(this.right, x.slice(i));
      (x = x.slice(0, i)).push(e);
      return make_sequence(this, x).optimize(compressor);
    }
  }
...
def_optimize(AST_Binary, function(self, compressor) {
  function reversible() {
    return (
      ...
      (!self.left.has_side_effects(compressor) && !self.right.has_side_effects(compressor))
    );
  }
  function reverse(op) {
    if (reversible()) {
  ...
  compressor.option("lhs_constants") && commutativeOperators.has(self.operator) &&
    self.right.is_constant() && !self.left.is_constant() &&
    ((self.left instanceof AST_Binary && PRECEDENCE[self.left.operator] >= PRECEDENCE[self.operator]) || reverse());
  ...
  case "!=":
    var expr;
    if (!is_strict_comparison && is_undefined(self.left, compressor)) self.left = make_node(AST_Null, self.left);
    else if (!is_strict_comparison && is_undefined(self.right, compressor)) self.right = make_node(AST_Null, self.right);
    else if (
      compressor.option("typeofs") &&
      ...
      if ((expr = self.right.expression) instanceof AST_SymbolRef ? expr.is_declared(compressor)
      ...
      if ((expr = self.left.expression) instanceof AST_SymbolRef ? expr.is_declared(compressor)
          : !(expr instanceof AST_PropAccess && compressor.option("ie8"))) {
        self.left = expr;
  ...
  if (compressor.option("evaluate")) {
    ...
    switch (self.operator) {
    case "&&":
      if (!(ll = !!has_flag(self.left, TRUTHY) || (!has_flag(self.left, FALSY) && self.left.evaluate(compressor)))) {
      ...
      if (!(rr = self.right.evaluate(compressor))) {
        ...
        set_flag(self, FALSY);
          ...
          (((parent = compressor.parent()).operator == "&&" && parent.left === compressor.self()) || compressor.in_boolean_context())) {
        compressor.warn("Dropping side-effect-free && [{file}:{line},{col}]", self.start);
        return self.left.optimize(compressor);
      }

      if (self.left.operator == "||" && !(lr = self.left.right.evaluate(compressor)))
    ...
    case "||":
      ...
      if (!(rr = self.right.evaluate(compressor))) {
        if (((parent = compressor.parent()).operator == "||" && parent.left === compressor.self()) || compressor.in_boolean_context()) {
          compressor.warn("Dropping side-effect-free || [{file}:{line},{col}]", self.start);
          return self.left.optimize(compressor);
        }
      } else if (!(rr instanceof AST_Node)) {
      ...
      if (self.left.operator == "&&" && (lr = self.left.right.evaluate(compressor)) && !(lr instanceof AST_Node))
        return make_node(AST_Conditional, self, {
          condition: self.left.left,
    ...
    case "??":
      ...
      if (!((ll = self.left.evaluate(compressor)) instanceof AST_Node)) return ll == null ? self.right : self.left;
    ...
    case "+":
      if (
        ...
        var l = (binary = make_node(AST_Binary, self, { operator: "+", left: self.left, right: self.right.left })).optimize(compressor);
        if (binary !== l) self = make_node(AST_Binary, self, { operator: "+", left: l, right: self.right.right });
      }
      if (
        ...
        var r = (binary = make_node(AST_Binary, self, { operator: "+", left: self.left.right, right: self.right })).optimize(compressor);
      ...
      if (
        ...
        var binary = make_node(AST_Binary, self, { operator: "+", left: self.left.right, right: self.right.left }),
      ...
      if (self.left instanceof AST_TemplateString) {
        l = self.left;
        if ((r = self.right.evaluate(compressor)) != self.right) {
      ...
      if (self.right instanceof AST_TemplateString) {
        r = self.right;
        if ((l = self.left.evaluate(compressor)) != self.left) {
    ...
    case "^":
      if (
        ...
        self = self.right instanceof AST_Constant && !(self.left instanceof AST_Constant)
          ? best_of(compressor, reversed, self)
          : best_of(compressor, self, reversed);
      }
    ...
    (lazy_op.has(self.operator) ||
      (self.operator == "+" &&
        (self.right.left.is_string(compressor) ||
          (self.left.is_string(compressor) && self.right.right.is_string(compressor)))))
  ) {
...
function within_array_or_object_literal(compressor) {
  for (var node, level = 0; (node = compressor.parent(level++)); ) {
    ...
    if (node instanceof AST_Array || node instanceof AST_ObjectKeyVal || node instanceof AST_Object) return true;
  }
  return false;
}
...
def_optimize(AST_SymbolRef, function(self, compressor) {
  ...
  if (compressor.option("reduce_vars") && is_lhs(self, parent) !== self) {
    ...
    var fixed = self.fixed_value(),
      single_use =
        def.single_use && !((parent instanceof AST_Call && parent.is_expr_pure(compressor)) || has_annotation(parent, _NOINLINE));
    if (single_use && (fixed instanceof AST_Lambda || fixed instanceof AST_Class))
      if (retain_top_func(fixed, compressor)) single_use = false;
      else if (def.scope !== self.scope &&
          (def.escaped == 1 || has_flag(fixed, INLINED) || within_array_or_object_literal(compressor)))
        single_use = false;
      else if (recursive_ref(compressor, def)) single_use = false;
      else if ((def.scope !== self.scope || def.orig[0] instanceof AST_SymbolFunarg) &&
          (single_use = fixed.is_constant_expression(self.scope)) == "f") {
        var scope = self.scope;
        do {
          if (scope instanceof AST_Defun || is_func_expr(scope)) set_flag(scope, INLINED);
    ...
    if (single_use && fixed instanceof AST_Lambda) {
      ...
      single_use =
        (def.scope === self.scope && !scope_encloses_variables_in_this_scope(block_scope, fixed)) ||
        (parent instanceof AST_Call && parent.expression === self && !scope_encloses_variables_in_this_scope(block_scope, fixed));
    }
    ...
    if (single_use && fixed) {
      ...
      if (def.recursive_refs > 0 && fixed.name instanceof AST_SymbolDefun) {
        ...
        let lambda_def = fixed.variables.get(fixed.name.name),
          name = lambda_def && lambda_def.orig[0];
      ...
      if (fixed instanceof AST_Lambda || fixed instanceof AST_Class) find_scope(compressor).add_child_scope(fixed);

      return fixed.optimize(compressor);
    }
    if (fixed && def.should_replace === void 0) {
      let init;
      if (fixed instanceof AST_This)
        def.orig[0] instanceof AST_SymbolFunarg || !def.references.every(ref => def.scope === ref.scope) || (init = fixed);
      else {
      ...
      if (init) {
        var fn,
          value_length = init.optimize(compressor).size();
...
def_optimize(AST_Undefined, function(self, compressor) {
  ...
  return lhs && is_atomic(lhs, self) ? self
    : make_node(AST_UnaryPrefix, self, { operator: "void", expression: make_node(AST_Number, self, { value: 0 }) });
});

def_optimize(AST_Infinity, function(self, compressor) {
  var lhs = is_lhs(compressor.self(), compressor.parent());
  return (lhs && is_atomic(lhs, self)) ||
    (compressor.option("keep_infinity") && (!lhs || is_atomic(lhs, self)) && !find_variable(compressor, "Infinity"))
    ? self
...
def_optimize(AST_NaN, function(self, compressor) {
  var lhs = is_lhs(compressor.self(), compressor.parent());
  return (lhs && !is_atomic(lhs, self)) || find_variable(compressor, "NaN")
    ? make_node(AST_Binary, self, {
        operator: "/",
...
const ASSIGN_OPS = makePredicate("+ - / * % >> << >>> | ^ &"),
  ASSIGN_OPS_COMMUTATIVE = makePredicate("* | ^ &");
def_optimize(AST_Assign, function(self, compressor) {
  ...
  if (
    ...
    do {
      node = parent;
      if ((parent = compressor.parent(level++)) instanceof AST_Exit) {
  ...
  if ((self = self.lift_sequences(compressor)).operator == "=" &&
      self.left instanceof AST_SymbolRef && self.right instanceof AST_Binary)
    if (self.right.left instanceof AST_SymbolRef &&
        self.right.left.name == self.left.name && ASSIGN_OPS.has(self.right.operator)) {
  ...
  function in_try(level, node) {
    ...
    self.right = right;
    for (var parent, scope = self.left.definition().scope; (parent = compressor.parent(level++)) !== scope; )
...
def_optimize(AST_DefaultAssign, function(self, compressor) {
  ...
  if (evaluateRight === void 0) self = self.left;
...
function is_nullish(node) {
  ...
  return (
    ...
    is_undefined(node) ||
    (node instanceof AST_SymbolRef && (fixed = node.definition().fixed) instanceof AST_Node && is_nullish(fixed))
  );
}
...
function is_nullish_check(check, check_subject, compressor) {
  ...
  if (
    ...
    check.operator === "==" &&
    ((nullish_side = is_nullish(check.left) && check.left) || (nullish_side = is_nullish(check.right) && check.right)) &&
  ...
  if (check instanceof AST_Binary && check.operator === "||") {
    ...
    const find_comparison = cmp => {
      ...
      return found === 1 && !!defined_side.equivalent_to(check_subject);
    };
...
def_optimize(AST_Conditional, function(self, compressor) {
  ...
  if (condition instanceof AST_SymbolRef && consequent instanceof AST_SymbolRef &&
  ...
  if (
    ...
    (!self.condition.has_side_effects(compressor) ||
      (consequent.operator == "=" && !consequent.left.has_side_effects(compressor)))
  )
  ...
  if (alternative instanceof AST_Conditional && consequent.equivalent_to(alternative.consequent))
    return make_node(AST_Conditional, self, {
      condition: make_node(AST_Binary, self, { operator: "||", left: condition, right: alternative.condition }),
      consequent: consequent,
      alternative: alternative.alternative
    }).optimize(compressor);

  if (compressor.option("ecma") >= 2020 && is_nullish_check(condition, alternative, compressor))
    return make_node(AST_Binary, self, { operator: "??", left: alternative, right: consequent }).optimize(compressor);
  ...
  if (alternative instanceof AST_Binary && alternative.operator == "&&" && consequent.equivalent_to(alternative.right))
    return make_node(AST_Binary, self, {
      operator: "&&",
      left: make_node(AST_Binary, self, { operator: "||", left: condition, right: alternative.left }),
      right: consequent
    }).optimize(compressor);
  ...
  return is_true(self.consequent)
    ? is_false(self.alternative)
      ? booleanize(self.condition)
      : make_node(AST_Binary, self, { operator: "||", left: booleanize(self.condition), right: self.alternative })
    : is_false(self.consequent)
    ? is_true(self.alternative)
      ? booleanize(self.condition.negate(compressor))
      : make_node(AST_Binary, self, { operator: "&&", left: booleanize(self.condition.negate(compressor)), right: self.alternative })
    : is_true(self.alternative)
    ? make_node(AST_Binary, self, { operator: "||", left: booleanize(self.condition.negate(compressor)), right: self.consequent })
    : is_false(self.alternative)
    ? make_node(AST_Binary, self, { operator: "&&", left: booleanize(self.condition), right: self.consequent })
    : self;

  function booleanize(node) {
    return node.is_boolean() ? node
      : make_node(AST_UnaryPrefix, node, { operator: "!", expression: node.negate(compressor) });
  }
  ...
  function is_true(node) {
    return (
      ...
      (node instanceof AST_UnaryPrefix && node.operator == "!" &&
        node.expression instanceof AST_Constant && !node.expression.getValue())
    );
  }
...
function safe_to_flatten(value, compressor) {
  ...
  return !!value && (
    !(value instanceof AST_Lambda || value instanceof AST_Class) ||
    !(value instanceof AST_Lambda && value.contains_this()) ||
  ...
  if (compressor.option("properties")) {
    ...
    if (key !== prop) {
      if (typeof key == "string")
        ...
        else if ((value = parseFloat(key)).toString() == key) key = value;

      prop = self.property = best_of_expression(prop, make_node_from_constant(key, prop).transform(compressor));
      var property = "" + key;
      if (is_basic_identifier_string(property) && property.length <= prop.size() + 1)
        return make_node(AST_Dot, self, { expression: expr, property: property, quote: prop.quote }).optimize(compressor);
    }
  }
  OPT_ARGUMENTS: if (
    ...
    for (var params = new Set(), argnames = fn.argnames, n = 0; n < argnames.length; n++) {
  ...
  if (compressor.option("properties") && compressor.option("side_effects") &&
    ...
    FLATTEN: if (safe_to_flatten(retValue, compressor)) {
      ...
      for (var i = elements.length; --i > index; )
        if ((value = elements[i].drop_side_effect_free(compressor))) {
      ...
      flatten || values.unshift(retValue);
      while (--i >= 0) {
...
AST_PropAccess.DEFMETHOD("flatten_object", function(key, compressor) {
  ...
  for (var props = expr.properties, i = props.length; --i >= 0; ) {
    var prop = props[i];
    if ("" + (prop instanceof AST_ConciseMethod ? prop.key.name : prop.key) != key) continue;
    if (!props.every(prop => prop instanceof AST_ObjectKeyVal || (arrows && prop instanceof AST_ConciseMethod && !prop.is_generator)))
      break;
...
def_optimize(AST_Dot, function(self, compressor) {
  ...
  if (compressor.option("unsafe_proto") && self.expression instanceof AST_Dot && self.expression.property == "prototype") {
    ...
    case "Object":
      ...
      break;
    case "RegExp":
      self.expression = make_node(AST_RegExp, self.expression, { value: { source: "t", flags: "" } });
      break;
    case "String":
...
function literals_in_boolean_context(self, compressor) {
  return compressor.in_boolean_context()
    ? best_of(compressor, self, make_sequence(self, [self, make_node(AST_True, self)]).optimize(compressor))
    : self;
}
  ...
  for (var props = self.properties, i = 0; i < props.length; i++) {
    ...
    if (prop instanceof AST_Expansion) {
      ...
      if (expr instanceof AST_Object) {
        ...
        i--;
      } else expr instanceof AST_Constant && !(expr instanceof AST_String) && props.splice(i, 1);
    }
  }
  return self;
});
...
def_optimize(AST_Function, function(self, compressor) {
  ...
  return !compressor.option("unsafe_arrows") ||
    ...
    ? self
    : make_node(AST_Arrow, self, self).optimize(compressor);
});
...
function lift_key(self, compressor) {
  ...
  if (self.key instanceof AST_String || self.key instanceof AST_Number) {
    ...
    if (self.key.value == "constructor" && compressor.parent() instanceof AST_Class) return self;
    self.key = self instanceof AST_ObjectKeyVal ? self.key.value
        : make_node(self instanceof AST_ClassProperty ? AST_SymbolClassProperty : AST_SymbolMethod, self.key, { name: self.key.value });
  }
  return self;
}
...
def_optimize(AST_ObjectKeyVal, function(self, compressor) {
  ...
  if (unsafe_methods && compressor.option("ecma") >= 2015 &&
      (!(unsafe_methods instanceof RegExp) || unsafe_methods.test(self.key + ""))) {
    var key = self.key,
      value = self.value;
    if (((value instanceof AST_Arrow && Array.isArray(value.body) && !value.contains_this()) ||
        value instanceof AST_Function) && !value.name)
...
def_optimize(AST_Destructuring, function(self, compressor) {
  if (
    compressor.option("pure_getters") == 1 &&
    compressor.option("unused") &&
    !self.is_array &&
  ...
  function should_retain(compressor, def) {
    return !!def.references.length ||
      (!!def.global &&
      (!compressor.toplevel.vars || (!!compressor.top_retain && compressor.top_retain(def))));
  }
});
...
function SourceMap(options) {
  ...
  return {
    add: add,
    get: () => generator,
    toString: () => JSON.stringify(generator.toJSON())
  };
}
...
function find_builtins(reserved) {
  ...
  var objects = {},
    global_ref = typeof global == "object" ? global : self;
  ...
  [
    ...
    ctor.prototype && Object.getOwnPropertyNames(ctor.prototype).map(add);
  });
  function add(name) {
    reserved.add(name);
  }
...
function reserve_quoted_keys(ast, reserved) {
  ...
  ast.walk(new TreeWalker(function(node) {
    node instanceof AST_ObjectKeyVal && node.quote ? add(node.key)
      : node instanceof AST_ObjectProperty && node.quote ? add(node.key.name)
      : node instanceof AST_Sub && addStrings(node.property, add);
  }));
}

function addStrings(node, add) {
  node.walk(new TreeWalker(function(node) {
    if (node instanceof AST_Sequence) addStrings(node.tail_node(), add);
    else if (node instanceof AST_String) add(node.value);
    else if (node instanceof AST_Conditional) {
...
function mangle_properties(ast, options) {
  ...
  var cache,
    cname = -1;
  options.cache
    ? (cache = options.cache.props).forEach(function(mangled_name) {
        reserved.add(mangled_name);
      })
    : (cache = new Map());

  var debug_name_suffix,
    regex = options.regex && new RegExp(options.regex),

    debug = options.debug !== false;
  if (debug) debug_name_suffix = options.debug === true ? "" : options.debug;

  var names_to_mangle = new Set(),
    unmangleable = new Set(),

    keep_quoted_strict = options.keep_quoted === "strict";

  ast.walk(new TreeWalker(function(node) {
    if (node instanceof AST_ObjectKeyVal) typeof node.key != "string" || (keep_quoted_strict && node.quote) || add(node.key);
    else if (node instanceof AST_ObjectProperty) (keep_quoted_strict && node.key.end.quote) || add(node.key.name);
    ...
    } else node instanceof AST_Sub ? keep_quoted_strict || addStrings(node.property, add)
      : node instanceof AST_Call && node.expression.print_to_string() == "Object.defineProperty" && addStrings(node.args[1], add);
  }));

  return ast.transform(new TreeTransformer(function(node) {
    node instanceof AST_ObjectKeyVal
      ? typeof node.key != "string" || (keep_quoted_strict && node.quote) || (node.key = mangle(node.key))
      : node instanceof AST_ObjectProperty
      ? (keep_quoted_strict && node.key.end.quote) || (node.key.name = mangle(node.key.name))
      : node instanceof AST_Dot ? (keep_quoted_strict && node.quote) || (node.property = mangle(node.property))
      : !options.keep_quoted && node instanceof AST_Sub ? (node.property = mangleStrings(node.property))
      : node instanceof AST_Call && node.expression.print_to_string() == "Object.defineProperty" &&
        (node.args[1] = mangleStrings(node.args[1]));
  }));

  function can_mangle(name) {
    return !unmangleable.has(name) &&
      !reserved.has(name) &&
      (options.only_cache ? cache.has(name) : !/^-?[0-9]+(\.[0-9]+)?(e[+-][0-9]+)?$/.test(name));
  }
  ...
  function add(name) {
    can_mangle(name) && names_to_mangle.add(name);

    should_mangle(name) || unmangleable.add(name);
  }
  ...
  function mangleStrings(node) {
    return node.transform(new TreeTransformer(function(node) {
      ...
      } else if (node instanceof AST_String) node.value = mangle(node.value);
      else if (node instanceof AST_Conditional) {
...
var to_ascii = typeof atob == "undefined" ? (b64) => Buffer.from(b64, "base64").toString() : atob,
  to_base64 = typeof btoa == "undefined" ? (str) => Buffer.from(str).toString("base64") : btoa;
  ...
  options[name] &&
    keys.forEach(function(key) {
      if (options[key]) {
        ...
        name in options[key] || (options[key][name] = options[name]);
      }
    });
}
...
function init_cache(cache) {
  if (cache)
    "props" in cache ? cache.props instanceof Map || (cache.props = map_from_object(cache.props)) : (cache.props = new Map());
}

function cache_to_json(cache) {
  return { props: map_to_object(cache.props) };
}
...
function minify(files, options) {
  ...
  try {
    ...
    var quoted_props,
      timings = options.timings && { start: Date.now() };
    if (options.keep_classnames === void 0) options.keep_classnames = options.keep_fnames;

    if (options.rename === void 0) options.rename = options.compress && options.mangle;
    ...
    if (options.mangle) {
      ...
      if (options.mangle.properties) {
        ...
        if (options.mangle.properties.keep_quoted) {
          ...
          Array.isArray(quoted_props) || (quoted_props = []);
    ...
    if (files instanceof AST_Toplevel) toplevel = files;
    else {
    ...
    quoted_props && options.mangle.properties.keep_quoted !== "strict" && reserve_quoted_keys(toplevel, quoted_props);
    ...
    // FIXME if (0 && options.rename);
    if (timings) timings.compress = Date.now();
    if (options.compress) toplevel = new Compressor(options.compress).compress(toplevel);
    if (timings) timings.scope = Date.now();
    options.mangle && toplevel.figure_out_scope(options.mangle);
    ...
    if (!HOP(options.output, "code") || options.output.code) {
      if (options.sourceMap) {
        if (typeof options.sourceMap.content == "string") options.sourceMap.content = JSON.parse(options.sourceMap.content);
      ...
      if (options.sourceMap) {
        result.map = options.sourceMap.asObject ? options.output.source_map.get().toJSON() : options.output.source_map.toString();
...
!(function() {
  var normalize_directives = function(body) {
    for (var in_directive = true, i = 0; i < body.length; i++)
      in_directive && body[i] instanceof AST_Statement && body[i].body instanceof AST_String
        ? (body[i] = new AST_Directive({ start: body[i].start, end: body[i].end, value: body[i].body.value }))
        : !in_directive || (body[i] instanceof AST_Statement && body[i].body instanceof AST_String) || (in_directive = false);
  ...
  var MOZ_TO_ME = {
    ...
    ArrayPattern: function(M) {
      return new AST_Destructuring({
        ...
        names: M.elements.map(function(elm) {
          return elm === null ? new AST_Hole() : from_moz(elm);
        }),
        is_array: true
      });
    },
    ...
    SpreadElement: function(M) {
      return new AST_Expansion({ start: my_start_token(M), end: my_end_token(M), expression: from_moz(M.argument) });
    },
    RestElement: function(M) {
      return new AST_Expansion({ start: my_start_token(M), end: my_end_token(M), expression: from_moz(M.argument) });
    },
    TemplateElement: function(M) {
      return new AST_TemplateSegment({ start: my_start_token(M), end: my_end_token(M), value: M.value.cooked, raw: M.value.raw });
    },
    TemplateLiteral: function(M) {
      ...
      return new AST_TemplateString({ start: my_start_token(M), end: my_end_token(M), segments: segments });
    },
    ...
    ArrowFunctionExpression: function(M) {
      const body = M.body.type === "BlockStatement" ? from_moz(M.body).body
    ...
    Property: function(M) {
      ...
      if (M.method) {
        ...
        M.computed ? (args.key = from_moz(M.key)) : (args.key = new AST_SymbolMethod({ name: args.key }));

        return new AST_ConciseMethod(args);
      }
      if (M.kind == "init") {
        if (key.type != "Identifier" && key.type != "Literal") args.key = from_moz(key);

        return new AST_ObjectKeyVal(args);
      }
      if (typeof args.key == "string" || typeof args.key == "number") args.key = new AST_SymbolMethod({ name: args.key });
    ...
    ObjectExpression: function(M) {
      return new AST_Object({
        ...
        properties: M.properties.map(function(prop) {
          if (prop.type === "SpreadElement") return from_moz(prop);
    ...
    ImportDeclaration: function(M) {
      ...
      M.specifiers.forEach(function(specifier) {
        ...
        } else if (specifier.type === "ImportDefaultSpecifier") imported_name = from_moz(specifier.local);
    ...
    ExportNamedDeclaration: function(M) {
      return new AST_Export({
        ...
        exported_names: M.specifiers && M.specifiers.length ? M.specifiers.map(function(specifier) {
          return new AST_NameMapping({ foreign_name: from_moz(specifier.exported), name: from_moz(specifier.local) });
        }) : null,
        module_name: from_moz(M.source)
      });
    },
    ...
    Literal: function(M) {
      var val = M.value, args = { start: my_start_token(M), end: my_end_token(M) },
        rx = M.regex;
      if (rx && rx.pattern) {
        args.value = { source: rx.pattern, flags: rx.flags };
        return new AST_RegExp(args);
      }
      if (rx) {
        const rx_source = M.raw || val,
          match = rx_source.match(/^\/(.*)\/(\w*)$/);
    ...
    MetaProperty: function(M) {
      if (M.meta.name === "new" && M.property.name === "target")
        return new AST_NewTarget({ start: my_start_token(M), end: my_end_token(M) });
    },
    ...
    BigIntLiteral: M => new AST_BigInt({ start: my_start_token(M), end: my_end_token(M), value: M.value })
  };

  MOZ_TO_ME.UpdateExpression = MOZ_TO_ME.UnaryExpression = function(M) {
    return new (("prefix" in M ? M.prefix : M.type == "UnaryExpression") ? AST_UnaryPrefix : AST_UnaryPostfix)({
      start: my_start_token(M),
      end: my_end_token(M),
      operator: M.operator,
  ...
  def_to_moz(AST_Expansion, function(M) {
    return { type: to_moz_in_destructuring() ? "RestElement" : "SpreadElement", argument: to_moz(M.expression) };
  });

  def_to_moz(AST_PrefixedTemplateString, function(M) {
    return { type: "TaggedTemplateExpression", tag: to_moz(M.prefix), quasi: to_moz(M.template_string) };
  });
  ...
  def_to_moz(AST_TemplateString, function(M) {
    ...
    for (var i = 0; i < M.segments.length; i++)
      i % 2 != 0 ? expressions.push(to_moz(M.segments[i])) : quasis.push({
        type: "TemplateElement",
        value: { raw: M.segments[i].raw, cooked: M.segments[i].value },
  ...
  def_to_moz(AST_Arrow, function(M) {
    var body = { type: "BlockStatement", body: M.body.map(to_moz) };
    return { type: "ArrowFunctionExpression", params: M.argnames.map(to_moz), async: M.async, body: body };
  });

  def_to_moz(AST_Destructuring, function(M) {
    return M.is_array ? { type: "ArrayPattern", elements: M.names.map(to_moz) }
      : { type: "ObjectPattern", properties: M.names.map(to_moz) };
  });

  def_to_moz(AST_Directive, function(M) {
    return {
      type: "ExpressionStatement",
      expression: { type: "Literal", value: M.value, raw: M.print_to_string() },
      directive: M.value
    };
  });
  ...
  def_to_moz(AST_Export, function(M) {
    return !M.exported_names ? {
      type: M.is_default ? "ExportDefaultDeclaration" : "ExportNamedDeclaration",
      declaration: to_moz(M.exported_value || M.exported_definition)
    } : M.exported_names[0].name.name === "*" ? {
      type: "ExportAllDeclaration",
      source: to_moz(M.module_name)
    } : {
      type: "ExportNamedDeclaration",
      specifiers: M.exported_names.map(function(name_mapping) {
        return { type: "ExportSpecifier", exported: to_moz(name_mapping.foreign_name), local: to_moz(name_mapping.name) };
      }),
  ...
  def_to_moz(AST_Import, function(M) {
    var specifiers = [];
    M.imported_name && specifiers.push({ type: "ImportDefaultSpecifier", local: to_moz(M.imported_name) });

    M.imported_names && M.imported_names[0].foreign_name.name === "*"
      ? specifiers.push({ type: "ImportNamespaceSpecifier", local: to_moz(M.imported_names[0].name) })
      : M.imported_names &&
        M.imported_names.forEach(function(name_mapping) {
          specifiers.push({ type: "ImportSpecifier", local: to_moz(name_mapping.name), imported: to_moz(name_mapping.foreign_name) });
        });

    return { type: "ImportDeclaration", specifiers: specifiers, source: to_moz(M.module_name) };
  });
  ...
  def_to_moz(AST_ObjectProperty, function(M, parent) {
    var kind,
      key = M.key instanceof AST_Node ? to_moz(M.key) : { type: "Identifier", value: M.key };
    if (typeof M.key == "number") key = { type: "Literal", value: Number(M.key) };

    if (typeof M.key == "string") key = { type: "Identifier", name: M.key };

    var string_or_num = typeof M.key == "string" || typeof M.key == "number",
      computed = !string_or_num && (!(M.key instanceof AST_Symbol) || M.key instanceof AST_SymbolRef);
    if (M instanceof AST_ObjectKeyVal) {
      kind = "init";
      computed = !string_or_num;
    } else if (M instanceof AST_ObjectGetter) kind = "get";
    else if (M instanceof AST_ObjectSetter) kind = "set";

    return M instanceof AST_ClassProperty
      ? { type: "FieldDefinition", computed, key, value: to_moz(M.value), static: M.static }

      : parent instanceof AST_Class
      ? { type: "MethodDefinition", computed, kind, static: M.static, key: to_moz(M.key), value: to_moz(M.value) }

      : { type: "Property", computed: computed, kind: kind, key: key, value: to_moz(M.value) };
  });
  ...
  def_to_moz(AST_Class, function(M) {
    return {
      ...
      body: { type: "ClassBody", body: M.properties.map(to_moz) }
    };
  });
  ...
  def_to_moz(AST_Symbol, function(M, parent) {
    if (M instanceof AST_SymbolMethod && parent.quote) return { type: "Literal", value: M.name };
  ...
  def_to_moz(AST_RegExp, function(M) {
    const pattern = M.value.source,
      flags = M.value.flags;
    return { type: "Literal", value: null, raw: M.print_to_string(), regex: { pattern, flags } };
  });

  def_to_moz(AST_Constant, function(M) {
    var value = M.value;
    return typeof value == "number" && (value < 0 || (value === 0 && 1 / value < 0))
      ? { type: "UnaryExpression", operator: "-", prefix: true, argument: { type: "Literal", value: -value, raw: M.start.raw } }
  ...
  AST_Hole.DEFMETHOD("to_mozilla_ast", () => null);

  AST_Block.DEFMETHOD("to_mozilla_ast", AST_BlockStatement.prototype.to_mozilla_ast);
  AST_Lambda.DEFMETHOD("to_mozilla_ast", AST_Function.prototype.to_mozilla_ast);

  function raw_token(moznode) {
    if (moznode.type == "Literal") return moznode.raw != null ? moznode.raw : moznode.value + "";
  }
  ...
  function map(moztype, mytype, propmap) {
    ...
    propmap && propmap.split(/\s*,\s*/).forEach(function(prop) {
  ...
  function set_moz_loc(mynode, moznode) {
    ...
    if (!start || !end) return moznode;

    if (start.pos != null && end.endpos != null) moznode.range = [start.pos, end.endpos];
  ...
  function to_moz_in_destructuring() {
    for (var i = TO_MOZ_STACK.length; i--; ) if (TO_MOZ_STACK[i] instanceof AST_Destructuring) return true;
  ...
  function to_moz_scope(type, node) {
    ...
    node.body[0] instanceof AST_SimpleStatement && node.body[0].body instanceof AST_String &&
...
exports._JS_Parse_Error = JS_Parse_Error;
exports._NOINLINE = _NOINLINE;
exports._PURE = _PURE;
exports._has_annotation = has_annotation;
exports._tokenizer = tokenizer;
