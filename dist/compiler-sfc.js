...
function replacer(_key, /** @prop {?boolean} __v_isRef */ val) {
 ...
 */
function noop(a, b, _c) { }
/**
 * Always return false.
 */
const no = (a, b, _c) => false;
            ...
            if (options.deindent === true ||
                ...
                (options.deindent !== false &&
                    (currentBlock.type !== 'script' ||
                        (currentBlock.lang && !/^[jt]sx?$/.test(currentBlock.lang))))) {
                text = deIndent(text);
            }
...
/**
 * @prop {{on: Function, off: Function}} _scope
 * @prop {?boolean} _isDestroyed
 * @prop {Function} $mount
...
class Dep {
    ...
    removeSub(sub) {
        ...
        // to work around this, we unset the sub for now, and clear them on
...
/**
 * @prop {Function} addDep
 * @prop {?Function} onTrack
 */
Dep.target = null;
const targetStack = [];
 ...
 */
function observe(/** @prop {?Observer} __ob__ */ value, shallow, ssrMockReactivity) {
...
function isReadonly(/** @prop {?boolean} __v_isReadonly */ value) {
...
// 1. When the children contain components - because a functional component
...
// 2. When the children contain constructs that always generated nested Arrays,
// e.g. <template>, <slot>, v-for, or when the children are provided by user
// with handwritten render functions / JSX. In such cases a full normalization
...
function registerDeepBindings(/** @prop {*} class */ data) {
...
function renderStatic(index, isInFor) {
    ...
    // if it has already-rendered static tree and not inside v-for,
...
function normalizeScopedSlots(ownerVm, scopedSlots, normalSlots, /** @prop {?boolean} $hasNormal */ prevScopedSlots) {
    let res;
 ...
 * @param {*} [_baseCtor]
 */
function resolveAsyncComponent(factory, _baseCtor) {
 ...
 * @prop {*} _vnode
 * @prop {Object} _props
 * @prop {Function} $forceUpdate
 * @prop {*} $parent
 * @prop {?boolean} _inactive
 * @prop {Array} $children
 * @prop {Object.<string, *>} $options
 * @prop {?boolean} _hasHookEvent
 * @prop {Function} $emit
 * @prop {?boolean} _isVue
 * @prop {Function} $createElement
 * @prop {*} _renderProxy
 * @prop {*} _attrsProxy
 * @prop {*} _listenersProxy
 * @prop {Object} _provided
 * @prop {*} $root
 * @prop {*} cid
 */
let target;
...
function updateChildComponent(vm, propsData, listeners, parentVnode, renderChildren) {
    ...
    // check if there are dynamic scopedSlots (handwritten or compiled but with
    ...
    // resolve slots + force update if it has children
    if (needsForceUpdate) {
...
function _traverse(/** @prop {?boolean} __v_skip */ val, seen) {
    let i, keys;
...
function resolveConstructorOptions(/** @prop {*} extendOptions */ Ctor) {
...
function parseFilters(exp) {
    ...
    let c = void 0, prev, i, expression, filters;
...
var he = (function() {
	...
	var encode = function(string, options) {
		...
		if (encodeEverything) {
			...
			// Note: any remaining non-ASCII symbols are handled outside the `if`.
	...
	var decode = function(html, options) {
		...
		return html.replace(regexDecode, function($0, $1, $2, $3, $4, $5, $6, $7, _$8) {
			var codePoint;
			var semicolon;
			var decDigits;
			var hexDigits;
...
function prefixIdentifiers(source, isFunctional = false, isTS = false, babelOptions = {}, bindings) {
    ...
    const ast = parser$1.parseExpression(source, Object.assign({}, babelOptions, { plugins }));
 ...
 * @param {SFCDescriptor} sfc
 * @param {SFCScriptCompileOptions} [options]
 */
function compileScript(sfc, options = { id: '' }) {
    ...
    if (!isTS || scriptLang === 'tsx' || scriptSetupLang === 'tsx') {
        ...
        // If it doesn't match the case of adding jsx, should remove the jsx from the babelParserPlugins
    ...
    if (!scriptSetup) {
        if (!script) {
            throw new Error('[@vue/compiler-sfc] SFC contains no \x3Cscript> tags.');
        }
        ...
        try {
            ...
            return Object.assign({}, script, { content,
                map,
    ...
    if (script && scriptLang !== scriptSetupLang) {
        throw new Error('[@vue/compiler-sfc] \x3Cscript> and \x3Cscript setup> must have the same language type.');
    }
    ...
    let propsDestructureDecl = void 0;
    let propsTypeDecl;
    ...
    function checkInvalidScopeReference(node, method) {
        ...
        walkIdentifiers(node, id => {
            if (setupBindings[id.name]) {
                error(`\`${method}()\` in \x3Cscript setup> cannot reference locally ` +
                    'declared variables because it will be hoisted outside of the ' +
                    'setup() function. If your component options require initialization ' +
                    'in the module scope, use a separate normal \x3Cscript> to export ' +
     ...
     * declarations. Otherwise, we will have to fall back to runtime merging.
     */
    ...
    if (script) {
        ...
        for (const node of scriptAst.body) {
            if (node.type === 'ImportDeclaration') {
                ...
                // export default
                // if it has name, skip name inference
                // if it has render and no template, generate return object instead of
    ...
    for (const node of scriptSetupAst.body) {
            ...
            walk$1(node, {
                enter(child, _parent) {
                    ...
                    if (child.type === 'AwaitExpression') {
                        error('Vue 2 does not support top level await in \x3Cscript setup>.', child);
                    }
                },
                /*exit*/ leave(node) {
        ...
        if ((node.type === 'ExportNamedDeclaration' && node.exportKind !== 'type') ||
            ...
            error('\x3Cscript setup> cannot contain ES module exports. ' +
                'If you are using a previous version of \x3Cscript setup>, please ' +
    ...
    const allBindings = Object.assign({}, scriptBindings, setupBindings);
    ...
    s.trim();
    return Object.assign({}, scriptSetup, {
...
function extractRuntimeProps(node, props, declaredTypes, _isProd) {
    const members = node.type === 'TSTypeLiteral' ? node.members : node.body;
    for (const m of members) {
...
function resolveTemplateUsageCheckString(sfc, isTS) {
    ...
    let code = '';
    parseHTML(content, Object.assign({}, baseOptions, {
        start(tag, attrs) {
...
function generateSourceMap(filename, source, generated, sourceRoot, pad) {
    const map = new (sourceMap$0().SourceMapGenerator)({
...
function urlToRequire(url, transformAssetUrlsOption = {}) {
    ...
    if (transformAssetUrlsOption.includeAbsolute ||
        ...
        if (!uriParts.hash) {
            ...
            // the requirement and instead appending it as string;
...
(function (_module, /** Object.<string, *> */ exports) {
	/*
	...
	exports.liquid.render = function(str, /** Object.<string, *> */ options, cb) {
	  return promisify(cb, function(cb) {
	    /** @type {Object.<string, *>} */
	...
	exports.dust.render = function(str, /** @prop {?string} views */ options, cb) {
	  return promisify(cb, function(cb) {
	...
	exports.handlebars.render = function(str, /** @prop {Object} helpers */ options, cb) {
	  return promisify(cb, function(cb) {
	...
	exports.nunjucks.render = function(str, /** Object.<string, *> */ options, cb) {
	  return promisify(cb, function(cb) {
	...
	function reactBaseTmpl(data, options) {
	  ...
	  // and interpolate / replace {{key}} with its value
	  for (var k in options) {
	...
	*/
	...
	function reactRenderer(type) {
	  ...
	  // Return rendering fx
	  return function(str, /** Object.<string, *> */ options, cb) {
	    return promisify(cb, function(cb) {
	      // React Import
	      /** @prop {Function} renderToStaticMarkup */
...
function genStatic(el, state) {
    ...
    // Some elements (templates) need to behave differently inside a v-pre
    ...
    if (el.directives) {
        for (let i = 0; i < el.directives.length; i++) {
            ...
            if (dir.name === 'model') {
                ...
                // remove value for textarea as it's converted to text
...
class Node {
    ...
    /**
     * When you want a value to pass through to CSS directly. This method
     * deletes the corresponding raw value causing the stringifier to fall back
...
class Attribute extends Namespace {
     ...
     *
     * @param {Object} [options]
     * @param {'"' | "'" | null} options.quoteMark - Use this value to quote the value. If this
     *     option is not set, the original value for quoteMark will be used. If
     *     indeterminate, a double quote is used. The legal values are:
     *     * `null` - the value will be unquoted and characters will be escaped as necessary.
     *     * `'` - the value will be quoted with a single quote and single quotes are escaped.
     *     * `"` - the value will be quoted with a double quote and double quotes are escaped.
     * @param {boolean} options.preferCurrentQuoteMark - if true, prefer the source quote mark
     *     over the quoteMark option value.
     * @param {boolean} options.smart - if true, will select a quote mark based on the value
    ...
    getQuotedValue (options = {}) {
        ...
        return cssesc_1(this._value, cssescOpts);
    }
     ...
     * @param {string} name One of the possible values inside an attribute.
...
function consumeEscape (css, start) {
    ...
    if (!unescapable[code]) if (hex[code]) {
        let hexDigits = 0;
...
class Parser {
     ...
     *
     * @param {*} nodes
     * @param {boolean} [requiredSpace=false]
     */
    ...
    combinator () {
        ...
        if (this.isNamedCombinator()) {
            ...
            this.position++;
        } else if (!WHITESPACE_TOKENS[this.currToken[FIELDS.TYPE]] && !spaceOrDescendantSelectorNodes) {
            this.unexpected();
        }
    ...
    parse (throwOnParenthesis) {
        ...
        case closeSquare:
            return this.missingSquareBracket();
        case semicolon:
            return this.missingBackslash();
        default:
...
class Processor {
     ...
     * @param rule {postcss.Rule | string} The css selector to be processed
     * @param [options] The options for processing
     * @returns {Promise<string>} the selector after processing.
     */
 ...
 *
 * @param {RawSourceMap|string} [oldMap] old source map object
 * @param {RawSourceMap|string} [newMap] new source map object
...
const sass = (source, map, options) => scss(source, map, Object.assign({}, options, { indentedSyntax: true }));
// .less
const less = (source, map, options) => {
    const nodeLess = require('less');
    let result;
    let error = null;
    nodeLess.render(getSource(source, options.filename, options.additionalData), Object.assign({}, options, { syncImport: true }), (err, output) => {
        error = err;
        result = output;
    });
    if (error)
