...
(exports = module.exports = CopyPlugin).default = CopyPlugin;
...
function preProcessPattern(globalRef, pattern) {
  ...
  pattern = typeof pattern == 'string' ? { from: pattern } : Object.assign({}, pattern);
  ...
  for (var index = 0; index < str.length; ) {
    ...
    if (closeSquareIndex !== -1 && str[index] === '[' && str[index + 1] !== ']') {
      ...
      if (closeSquareIndex > index &&
          ...
          (backSlashIndex = str.indexOf('\\', index)) === -1 || backSlashIndex > closeSquareIndex))
        return true;
    }

    if (closeCurlyIndex !== -1 && str[index] === '{' && str[index + 1] !== '}' &&
        (closeCurlyIndex = str.indexOf('}', index)) > index &&
        ((backSlashIndex = str.indexOf('\\', index)) === -1 || backSlashIndex > closeCurlyIndex))
      return true;

    if (closeParenIndex !== -1 && str[index] === '(' && str[index + 1] === '?' &&
        /[:!=]/.test(str[index + 2]) && str[index + 3] !== ')' &&
        (closeParenIndex = str.indexOf(')', index)) > index &&
        ((backSlashIndex = str.indexOf('\\', index)) === -1 || backSlashIndex > closeParenIndex))
      return true;
    ...
    if (pipeIndex !== -1 && str[index] === '(' && str[index + 1] !== '|') {
      ...
      if (pipeIndex !== -1 && str[pipeIndex + 1] !== ')' &&
          ...
          ((backSlashIndex = str.indexOf('\\', pipeIndex)) === -1 || backSlashIndex > closeParenIndex))
        return true;
    }
...
function uniqNoSet(arr) {
  ...
  for (var i = 0; i < arr.length; i++) ret.indexOf(arr[i]) !== -1 || ret.push(arr[i]);
...
const getIsIgnoredPredecate = (ignores, cwd) => p => ignores.ignores(slash(path.relative(cwd, p)));
...
module.exports.sync = o => {
  const opts = normalizeOpts(o),
    ...
    paths = glob.sync('**/.gitignore', { ignore: opts.ignore, cwd: opts.cwd }),
    files = paths.map(file => getFileSync(file, opts.cwd)),
...
var REGEX_BLANK_LINE = /^\s+$/,
  REGEX_LEADING_EXCAPED_EXCLAMATION = /^\\\!/,
  REGEX_LEADING_EXCAPED_HASH = /^\\#/,
  SLASH = '/',
