...
const util = require("util"),
	...
	path = require("path"),
	/** @type {import("fs")} */ fs = require("../vendor/graceful-fs");

function createInnerContext(options, message, _messageOptional) {
...
function startsWith(/** string */ string, searchString) {
		...
		resolver
			...
			.tapAsync("AliasFieldPlugin", (request, resolveContext, callback) => {
				...
				resolver.doResolve(
					target,
					obj,
					"aliased from description file " + request.descriptionFilePath +
						" with mapping '" + innerRequest + "' to '" + data + "'",
					resolveContext,
					(err, result) => {
						if (err) return callback(err);
	...
	constructor(fileSystem, duration) {
		...
		if (this.fileSystem.readJson)
			...
			this._readJson = (path, callback) => {
				this.readFile(path, (err, /** Buffer */ buffer) => {
					if (err) return callback(err);
					let data;
					try {
